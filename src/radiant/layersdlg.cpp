...
// ══════════════════════════════════════════════════════════════════════════════
//  Deterministic test entry (the `layers` gate).  Drives the dialog's CORE ops
//  WITHOUT any HWNDs: enumerate → create a new layer → assign the first active
//  world brush to it → hide it.  The gate (radiantapp.cpp RunLayersTest) then
//  saves + reloads + checks the .map carries both the new layer block line AND the
//  per-brush `layer "<name>"` line, and the hidden flag round-trips.
//
//  Returns: layer count BEFORE the new layer was added (>0 proves enumeration saw
//  the map's layers), or -1 if no assignable world brush exists on this map.
// ══════════════════════════════════════════════════════════════════════════════
extern void Select_Brush( selbrush_t *b, char some_overwrite, char bStatus, char center ); // select.cpp 0x48DCC0
extern void Select_Deselect( int keep );                                                   // select.cpp 0x48E800

extern "C" int Radiant_TestLayers( const char *newLayerName )
{
    // 1) enumeration — how many layers did the map declare?
    std::vector<std::pair<std::string,int>> before;
    Layers_Enumerate( before );
    int beforeCount = (int)before.size();

    // 2) find the first active WORLD brush (non-prefab, non-patch, def present) and
    //    select it.  (Like the texmod/findrepl helpers, a prefab-only map — mp_backlot
    //    — has no inline brush to assign, so we return -1 = N/A.)
    selbrush_t *target = nullptr;
    ...
    // 3) create the new layer (Layers_AddLayerPath via the dialog's OnNew core).
    Layers_AddLayerPath( newLayerName, 0 );

    // 4) assign the selected brush to it (AssignSelectionToLayer core).
    Select_Deselect( 1 );
    Select_Brush( target, 0, 0, 0 );
    Layers_AssignSelectionToLayer( newLayerName );

    // 5) hide the layer (the persisted flag + the runtime FilterBrush toggle).
    Layers_SetHidden( newLayerName, true );

    Select_Deselect( 1 );   // leave the selection clean for the save
    return beforeCount;
}
