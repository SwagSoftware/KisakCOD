...
// ══════════════════════════════════════════════════════════════════════════════
//  Deterministic test entry points (the `texbar` gate).  Drive the texture bar's READ /
//  WRITE texdef halves WITHOUT any HWNDs — the DEF-based core that works headlessly (the
//  spin nudges go through Brush_*Texture, which loops the instance faceCount that is only
//  built by a camera draw; the read/write halves operate on the def directly, like the
//  surfacedlg texmod gate).  RunTexBarTest (radiantapp.cpp) selects a face, reads the bar
//  values, writes new ones, saves + reloads, and asserts the texdef round-trips.
// ══════════════════════════════════════════════════════════════════════════════

// Read the picked face's texdef as the bar would display it: out = {shiftH, shiftV,
// scaleH, scaleV, rotate} (truncated ints, the m_n* member values).  Returns 0 if no face.
extern "C" int Radiant_TestTexBarRead( int out[5] )
{
    if ( SEL_FACE_COUNT() <= 0 )
        return 0;
    texdef_sub_t *td = TexBar_TargetTexdef();
    if ( !td )
        return 0;
    out[0] = (int)td->shift[0];   // m_nHShift
    out[1] = (int)td->shift[1];   // m_nVShift
    out[2] = (int)td->size[0];    // m_nHScale
    out[3] = (int)td->size[1];    // m_nVScale
    out[4] = (int)td->rotate;     // m_nRotate
    return 1;
}

// Write the bar's member ints back onto the picked face's texdef (TextureBar_02 core,
// the field→texdef store) and invalidate.  Returns 0 if no face selected.
extern "C" int Radiant_TestTexBarWrite( int shiftH, int shiftV, int scaleH, int scaleV, int rotate )
{
    if ( SEL_FACE_COUNT() <= 0 )
        return 0;
    texdef_sub_t *td = TexBar_TargetTexdef();
    if ( !td )
        return 0;
    td->shift[0] = (float)shiftH;
    td->shift[1] = (float)shiftV;
    td->size[0]  = (float)scaleH;
    td->size[1]  = (float)scaleV;
    td->rotate   = (float)rotate;
    g_nUpdateBits |= 1;
    return 1;
}
