...
// Framing-independent count of the entity-name labels the draw would emit: both brush lists
// under the SAME representative-brush predicate, without the view cull or the names-shown
// flag.  The predicate fires once per entity, so this == the number of distinct non-world
// entities with a labelled brush.
static int Ed_CountEntityNameLabels()
{
    int n = 0;
    for ( selbrush_t *b = active_brushes.next; b && b != &active_brushes; b = b->next )
        if ( Ed_EntityNameForBrush( b ) )
            ++n;
...
// Headless gate entry (radiantapp.cpp RunEntityNamesTest); -1 = brush lists not bootstrapped.
extern "C" int Radiant_TestEntityNameCount()
{
    if ( !active_brushes.next )
        return -1;
    return Ed_CountEntityNameLabels();
}
...
// Connections gate: count the target/targetname and script_linkTo/script_linkName link PAIRS
// the overlay would draw, replicating Lines_AddLinkTo/Script's MATCH loops without touching
// the renderer (the headless selftest has no live D3D device).  0 is a valid PASS (= no
// connections on this map).
extern "C" int Radiant_TestConnectionLinks( int *outTargetLinks, int *outScriptLinks )
{
    int tLinks = 0, sLinks = 0;

    // target/targetname pairs (Lines_AddLinkTo's match loop).  Brush-visibility aside: the
    // gate counts the logical links regardless of filter state.
    for ( entity_s *te = entityInsts.next; te != &entityInsts; te = te->next )
    {
        const char *target = Ed_EntFirstKey( te, "target" );
        if ( !*target || !Ed_EntFirstBrushInst( te ) )
            continue;
        for ( entity_s *me = entityInsts.next; me != &entityInsts; me = me->next )
        {
            if ( me == te ) continue;
            const char *tn = Ed_EntFirstKey( me, "targetname" );
            if ( *tn && !strcmp( tn, target ) && Ed_EntFirstBrushInst( me ) )
                ++tLinks;
        }
    }

    // script_linkTo/script_linkName pairs (Lines_AddLinkToScript's match loop)
    for ( entity_s *te = entityInsts.next; te != &entityInsts; te = te->next )
    {
        const char *linkTo = Ed_EntFirstKey( te, "script_linkTo" );
        if ( !*linkTo || !Ed_EntFirstBrushInst( te ) )
            continue;
        int ids[32];
        int n = Ed_ParseLinkList( ids, linkTo );
        for ( entity_s *me = entityInsts.next; me != &entityInsts; me = me->next )
        {
            int v = atol( Ed_EntFirstKey( me, "script_linkName" ) );
            if ( !v || !Ed_EntFirstBrushInst( me ) )
                continue;
            for ( int k = 0; k < n; ++k )
                if ( ids[k] == v ) { ++sLinks; break; }
        }
    }

    if ( outTargetLinks ) *outTargetLinks = tLinks;
    if ( outScriptLinks ) *outScriptLinks = sLinks;
    return tLinks + sLinks;
}
#endif

#ifdef RADIANT_SELFTEST
// Clip gate: select the first editable world brush, place 3 explicit clip points that BISECT
// it along its mid-X (determinate regardless of the d_new_brush template), run the real
// Ed_ProduceSplitLists, and report the front/back split-brush counts.
extern "C" void *Radiant_TestClipSplit( int *outFront, int *outBack )
{
    *outFront = *outBack = 0;
    ...
    const float *mins = pick->def->mins;
    const float *maxs = pick->def->maxs;
    const float midX = 0.5f * ( mins[0] + maxs[0] );

    Ed_SetClipMode( 1 );           // resets the 3 points + the split lists
    g_Clip1.m_ptClip[0] = midX; g_Clip1.m_ptClip[1] = mins[1]; g_Clip1.m_ptClip[2] = mins[2]; g_Clip1.m_bSet = true;
    g_Clip2.m_ptClip[0] = midX; g_Clip2.m_ptClip[1] = maxs[1]; g_Clip2.m_ptClip[2] = mins[2]; g_Clip2.m_bSet = true;
    g_Clip3.m_ptClip[0] = midX; g_Clip3.m_ptClip[1] = mins[1]; g_Clip3.m_ptClip[2] = maxs[2]; g_Clip3.m_bSet = true;

    Ed_ProduceSplitLists();

    int nf = 0, nb = 0;
    for ( selbrush_t *b = g_brFrontSplits.next; b && b != &g_brFrontSplits; b = b->next ) ++nf;
    for ( selbrush_t *b = g_brBackSplits.next;  b && b != &g_brBackSplits;  b = b->next ) ++nb;
    *outFront = nf;
    *outBack  = nb;
    return pick->def;
}

// Commit the clip (keep one side) via the real Ed_Clip + the menu's undo bracket. Builds
// its own synthetic XY-view CXYWnd (Ed_Clip only reads m_nViewType). Returns 1.
extern "C" int Radiant_TestClipCommit()
{
    if ( !g_bClipMode )
        return 0;
    CXYWnd wnd;                       // no Create(): m_hWnd stays null -> dtor is a no-op
    wnd.m_nViewType = ED_VIEW_XY;
    Undo_ClearRedo();
...
#ifdef RADIANT_SELFTEST
// Entity-creation gate: a placeholder brush must already be selected (Radiant_TestCreateBrushDrag
// makes one); CreateEntityFromName then consumes it and drops a point entity of `classname`.
extern "C" void *Radiant_TestCreateEntity( const char *classname )
{
    CreateEntityFromName( classname );
    if ( selected_brushes.next == &selected_brushes )
        return nullptr;
    return (void *)selected_brushes.next->owner;     // the created entity instance
}

// Re-class gate (CreateEntityFromName Path B): with a single fixedsize point entity's bbox
// selected, call CreateEntityFromName with a SECOND fixedsize class - Entity_Create refuses,
// so Path B re-classes in place.  Verifies the eclass switched, modelClass cleared, the bbox
// rebuilt to the new eclass dims, and exactly one brush selected afterwards (outOk).
extern "C" void *Radiant_TestReclassEntity( const char *newClass, int *outOk )
{
    if ( outOk ) *outOk = 0;

    // Capture the pre-reclass state (the single selected entity's bbox).
    if ( selected_brushes.next == &selected_brushes ||
         selected_brushes.next->next != &selected_brushes )
        return nullptr;                                  // need exactly one selected brush
    selbrush_t   *preInst = selected_brushes.next;
    entity_s     *ent     = preInst->owner;
    entity_s_def *eDef    = (entity_s_def *)ent->def;
    if ( !eDef || !eDef->eclass )
        return nullptr;

    eclass_t *newEc = Eclass_ForName( 0, newClass );
    if ( !newEc || !*(int *)&newEc->fixedsize )
        return nullptr;                                  // gate requires a fixedsize target

    // Drive Path B.
    CreateEntityFromName( newClass );

    // After the re-class: one brush must be selected, owned by `ent`, def == new bbox.
    if ( selected_brushes.next == &selected_brushes ||
         selected_brushes.next->next != &selected_brushes )
        return nullptr;
    selbrush_t   *postInst = selected_brushes.next;
    entity_s     *postEnt  = postInst->owner;
    entity_s_def *postDef  = (entity_s_def *)postEnt->def;
    brush_t      *nb       = postInst->def;

    int ok = 1;
    if ( postEnt != ent )                          ok = 0;   // same entity object
    if ( postDef->eclass != newEc )                ok = 0;   // eclass switched
    if ( postDef->modelClass != nullptr )          ok = 0;   // modelClass cleared
    if ( !nb )                                     ok = 0;
    else
    {
        // The rebuilt bbox brush extents must equal newEclass's dimensions
        // (mins/maxs shifted by the same delta keep the dims invariant).
        for ( int i = 0; i < 3; ++i )
        {
            float dimNew = nb->maxs[i] - nb->mins[i];
            float dimEc  = newEc->maxs[i] - newEc->mins[i];
            if ( dimNew - dimEc > 0.01f || dimEc - dimNew > 0.01f )
                ok = 0;
        }
    }
    if ( postDef->brushes.prev != (selbrush_t *)nb ) ok = 0; // entity's first def is nb

    if ( outOk ) *outOk = ok;
    return (void *)postEnt;
}

// Align-to-floor gate: AlignEntityToFace_OrientToFloor (0x485AD0) -> sub_4859B0.  Sets the
// entity's "angles", aligns onto a tilted floor normal, and verifies the rebuilt forward is
// unit-length AND perpendicular to that normal (sub_4859B0's defining invariant).
extern void AlignEntityToFace_OrientToFloor( entity_s_def *ent, float *dir );  // entity.cpp 0x485AD0
extern int  Entity_GetVec3ForKey( entity_s_def *e, float *out, const char *key ); // entity.cpp
extern "C" int Radiant_TestAlignToFloor( void *entInst, int *outFixed, int *outPerp )
{
    *outFixed = *outPerp = 0;
    if ( !entInst )
        return 0;
    entity_s     *owner = (entity_s *)entInst;
    entity_s_def *def   = (entity_s_def *)owner->def;
    if ( !def || !def->eclass )
        return 0;
    *outFixed = *(int *)&def->eclass->fixedsize ? 1 : 0;

    SetKeyValue( def, "angles", "20 50 0" );

    float dir[3] = { 0.1f, 0.2f, 1.0f };               // a tilted "floor" normal
    float dlen = (float)sqrt( dir[0]*dir[0] + dir[1]*dir[1] + dir[2]*dir[2] );
    dir[0] /= dlen; dir[1] /= dlen; dir[2] /= dlen;

    AlignEntityToFace_OrientToFloor( def, dir );

    float ang[3] = { 0.0f, 0.0f, 0.0f };
    Entity_GetVec3ForKey( def, ang, "angles" );
    const float p = ang[0] * 0.01745329238474369f;     // pitch
    const float y = ang[1] * 0.01745329238474369f;     // yaw
    const float cp = (float)cos( p ), sp = (float)sin( p );
    const float cy = (float)cos( y ), sy = (float)sin( y );
    float fwd[3] = { cp * cy, cp * sy, -sp };           // forward the angles encode

    float dot  = fwd[0]*dir[0] + fwd[1]*dir[1] + fwd[2]*dir[2];   if ( dot < 0 ) dot = -dot;
    float flen = (float)sqrt( fwd[0]*fwd[0] + fwd[1]*fwd[1] + fwd[2]*fwd[2] );
    *outPerp = ( dot < 5e-3f && flen > 0.99f && flen < 1.01f ) ? 1 : 0;
    return 1;
}

// DoFlip gate (0x424F30): mirror the selection AND flip the entity's "angles" by 180 deg on
// the mirror axis.  Angles (10,30,50), flip axis Y -> yaw must become
// AngleNormalize360(30+180)=210 with pitch/roll untouched.
extern void DoFlip( int axis, const char *opName );   // select.cpp 0x424F30
extern "C" int Radiant_TestDoFlip( void *entInst, int *outFlippedOk, int *outOthersKept )
{
    *outFlippedOk = *outOthersKept = 0;
    if ( !entInst )
        return 0;
    entity_s     *owner = (entity_s *)entInst;
    entity_s_def *def   = (entity_s_def *)owner->def;
    ...
    SetKeyValue( def, "angles", "10 30 50" );   // pitch 10, yaw 30, roll 50
    DoFlip( 1, "flip Y" );                        // mirror + flip ang[1] (yaw): 30 -> 210

    float ang[3] = { 0.0f, 0.0f, 0.0f };
    Entity_GetVec3ForKey( def, ang, "angles" );
    float dyaw = ang[1] - 210.0f; if ( dyaw < 0 ) dyaw = -dyaw;
    *outFlippedOk = ( dyaw <= 1e-2f ) ? 1 : 0;
    float dp = ang[0] - 10.0f; if ( dp < 0 ) dp = -dp;     // pitch unchanged
    float dr = ang[2] - 50.0f; if ( dr < 0 ) dr = -dr;     // roll unchanged
    *outOthersKept = ( dp <= 1e-2f && dr <= 1e-2f ) ? 1 : 0;
    return 1;
}

// Brush-creation gate: drive the REAL NewBrushDrag chain on a synthetic CXYWnd (XY top view,
// no HWND).  Move 1 CREATES a brush sized to (press->cur1); move 2 RESIZES it to (press->cur2)
// through the now-selected brush.
extern "C" void *Radiant_TestCreateBrushDrag(
    int px, int py, int cx1, int cy1, int cx2, int cy2,
    float scale, float ox, float oy, int width, int height )
{
    CXYWnd wnd;                       // no Create(): m_hWnd stays null → dtor is a no-op
    wnd.m_nViewType        = ED_VIEW_XY;
    wnd.m_fScale           = scale;
    wnd.m_vOrigin[0]       = ox;  wnd.m_vOrigin[1] = oy;  wnd.m_vOrigin[2] = 0.0f;
    wnd.m_nWidth           = width;
    wnd.m_nHeight          = height;
    wnd.m_nButtonstate     = MK_LBUTTON;
    wnd.m_nPressx          = px;
    wnd.m_nPressy          = py;
    wnd.m_vPressdelta[0]   = wnd.m_vPressdelta[1] = wnd.m_vPressdelta[2] = 0.0f;
    wnd.m_bPress_selection = false;   // nothing selected at press → the create branch

    Ed_NewBrushDrag( &wnd, cx1, cy1 );          // CREATE (Brush_Alloc/Create/Link/Select)
    if ( cx2 != cx1 || cy2 != cy1 )
        Ed_NewBrushDrag( &wnd, cx2, cy2 );      // RESIZE the selected brush (Brush_Build)

    return ( selected_brushes.next != &selected_brushes ) ? (void *)selected_brushes.next
                                                          : nullptr;
}

// Brush-entity gate: drive Entity_Create directly with a NON-fixedsize (brush) eclass, which
// exercises the reparent loop (each selected WORLD brush moves out of worldspawn into the new
// entity).  A placeholder world brush must be selected.  Direct Entity_Create, not
// CreateEntityFromName, so the point-entity bbox tail is skipped.
extern "C" void *Radiant_TestCreateBrushEntity( const char *classname )
{
    // Eclass_ForName ignores has_brushes when the class is already in g_eclass (the GUI
    // always has cod4.def fully loaded, so the real callers' has_brushes=0 is moot there);
    // when NOT pre-loaded (headless selftest on a map that never referenced this class) it
    // SYNTHESIZES a placeholder whose fixedsize tracks has_brushes — so pass 1 here to get a
    // BRUSH-typed eclass (fixedsize=0), matching the real cod4.def brush class.
    eclass_t *ec = Eclass_ForName( 1, classname );
    if ( !ec )                       return nullptr;
    if ( *(int *)&ec->fixedsize )    return nullptr;   // not a brush entity — wrong gate input
    return (void *)Entity_Create( ec );
}
#endif // RADIANT_SELFTEST
