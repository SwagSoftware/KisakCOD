...
// ══════════════════════════════════════════════════════════════════════════════
//  Deterministic test entries.  Drive the CORES with no HWNDs (the find_brush /
//  arb_rotate gates in radiantapp.cpp):
//   * Radiant_TestSelectByNumber(brushIdx,entIdx) → Select_ByEntityNumber, returns the
//     selected-brush count (1 = selected something) so the gate can assert "selected the
//     intended brush" by then reading GetSelectionIndex back.
//   * Radiant_TestGetSelectionIndex(&ent,&brush) → GetSelectionIndex passthrough.
//   * Radiant_TestArbitraryRotate(x,y,z) → the per-axis rotate path (no dialog), exactly
//     as the menu handler applies it (used by the rotation round-trip gate).
// ══════════════════════════════════════════════════════════════════════════════
extern "C" int Radiant_TestSelectByNumber( int brushIdx, int entIdx )
{
    Select_ByEntityNumber( brushIdx, entIdx );
    int n = 0;
...
extern "C" int Radiant_TestGetSelectionIndex( int *entOut, int *brushOut )
{
    GetSelectionIndex( entOut, brushOut );
    return 0;
}

extern "C" int Radiant_TestArbitraryRotate( float x, float y, float z )
{
    if ( selected_brushes.next == &selected_brushes )
        return -1;
    Undo_ClearRedo();
    Undo_GeneralStart( "arbitrary rotation" );
    Undo_AddBrushList( &selected_brushes );
    ArbRot_ApplyAxis( 0, x );
    ArbRot_ApplyAxis( 1, y );
    ArbRot_ApplyAxis( 2, z );
    UpdateSelection( -1, 0 );
    Undo_EndBrushList( &selected_brushes );
    Undo_End();
    return 0;
...
// ── the find-brush self-inverse gate (the `findbrush` CLI command) ────────────
// GetSelectionIndex and Select_ByEntityNumber are exact inverses: select a brush, read
// its (entity#, brush#), deselect, then Select_ByEntityNumber(brush#, entity#) must
// reselect THE SAME brush.  This drives both new cores end-to-end with no HWNDs.
//   out[0] = entity index reported for the picked brush (GetSelectionIndex)
//   out[1] = brush index reported for the picked brush
//   out[2] = selected-brush count after re-select-by-number (expect 1)
//   out[3] = 1 iff the SAME brush def was reselected (the round-trip is faithful)
// Returns 0 on success, -1 = N/A (no editable inline world brush — pure-prefab map).
extern "C" int Radiant_TestFindBrushRoundTrip( int *out /* int[4] */ )
{
    out[0] = out[1] = out[2] = out[3] = 0;

    // Pick an editable, non-patch world brush (same filter as the transform gate).
    ...
    // Deselect, then re-select purely by number.
    Select_Deselect( 1 );
    Select_ByEntityNumber( brushIdx, entIdx );
