...
// Deterministic face-selection test entry (the texmod gate) — thin wrapper.
extern "C" int Radiant_TestSelectFace() { return Ed_SelectFirstEditableFace(); }

// invertsel gate: Select_Invert (0x493F10).  Invert twice from the post-load state and
// check the wholesale list swap, the double-invert identity, and list-count conservation.
// out = {active,sel} after 0/1/2 inverts.
static int ST_CountBrushList( selbrush_t *sentinel )
{
    int n = 0;
    for ( selbrush_t *b = sentinel->next; b && b != sentinel; b = b->next )
        if ( ++n > 1000000 ) break;                 // runaway guard (corrupt list)
    return n;
}
extern "C" int Radiant_TestInvertSelection( int *a0, int *s0, int *a1, int *s1, int *a2, int *s2 )
{
    *a0 = ST_CountBrushList( &active_brushes );  *s0 = ST_CountBrushList( &selected_brushes );
    Select_Invert();
    *a1 = ST_CountBrushList( &active_brushes );  *s1 = ST_CountBrushList( &selected_brushes );
    Select_Invert();
    *a2 = ST_CountBrushList( &active_brushes );  *s2 = ST_CountBrushList( &selected_brushes );
    return 1;
}

// classsimilar gate: Select_ByClassSimilar (0x493830).  Regression guard for the
// head-vs-tail iteration bug - the binary starts at the list TAIL and walks ->prev, so a
// port starting at .next matches only the head brush's class.  Selects one brush of each
// of two distinct eclasses, runs the command, and reports how many of each class ended up
// selected.  Returns 0 (N/A) if the map lacks two >=2-member entity classes.
void Select_Deselect( int bDeselectFaces );   // defined below (0x48E800)
void Select_ByClassSimilar();                 // defined below (0x493830)
static const char *ST_BrushClassName( selbrush_t *b )
{
    if ( !b || !b->owner || b->owner == world_entity ) return nullptr;
    entity_s_def *d = (entity_s_def *)b->owner->def;
    if ( !d || !d->eclass || !d->eclass->name ) return nullptr;
    return d->eclass->name;
}
extern "C" int Radiant_TestClassSimilar( int *outClasses, int *outSelCount,
                                         int *outSelA, int *outSelB )
{
    Select_Deselect( 1 );

    // Both classes need >=2 selectable active brushes: whichever ends up at the selection
    // TAIL is the node the buggy .next-start walk skips, so requiring BOTH >=2 catches the
    // bug regardless of Brush_AddToList2 order.
    char clsA[256] = "", clsB[256] = "";
    selbrush_t *brA = nullptr, *brB = nullptr;
    for ( selbrush_t *b = active_brushes.next; b != &active_brushes; b = b->next )
    {
        const char *nm = ST_BrushClassName( b );
        if ( !nm ) continue;
        if ( *clsA && !_stricmp( nm, clsA ) ) continue;   // already chosen as A
        int cnt = 0;
        for ( selbrush_t *c = active_brushes.next; c != &active_brushes; c = c->next )
        {
            const char *cn = ST_BrushClassName( c );
            if ( cn && !_stricmp( cn, nm ) && !FilterBrush( c, 0 ) ) ++cnt;
        }
        if ( cnt < 2 ) continue;                          // need >=2 matchable members
        if ( !*clsA )      { strncpy( clsA, nm, sizeof(clsA)-1 ); brA = b; }
        else               { strncpy( clsB, nm, sizeof(clsB)-1 ); brB = b; break; }
    }
    *outClasses = ( *clsA ? 1 : 0 ) + ( *clsB ? 1 : 0 );
    *outSelCount = *outSelA = *outSelB = 0;
    if ( !*clsA || !*clsB ) return 0;   // map lacks two distinct >=2-member classes → N/A

    Select_Brush( brA, 0, 0, 0 );
    Select_Brush( brB, 0, 0, 0 );
    ...
    for ( selbrush_t *b = selected_brushes.next; b != &selected_brushes; b = b->next )
    {
        const char *nm = ST_BrushClassName( b );
        ++*outSelCount;
        if ( nm && !_stricmp( nm, clsA ) ) ++*outSelA;
        if ( nm && !_stricmp( nm, clsB ) ) ++*outSelB;
    }
    return 1;
}

// prim gate harness: select the first editable WORLD brush (single selection, the
// QE_SingleBrush precondition the cone/sphere require), then reshape it into the
// requested primitive.  kind 0 = sphere (Brush_MakeSidedSphere), 1 = cone
// (Brush_MakeSidedCone).  Returns the resulting def faceCount (0 if no brush was
// found / the op refused).  The brush stays in selected_brushes; the gate saves +
// reloads to prove the round-trip.
extern void Brush_MakeSided( int b, unsigned int sides, int axis, char snap ); // brush.cpp (0x4731E0)
...
extern "C" int Radiant_TestMakePrimitive( int kind, int sides )
{
    ...
    for ( selbrush_t *b = active_brushes.next; b && b != &active_brushes; b = b->next )
    {
        if ( b->patch || !b->def || b->def->faceCount <= 0 )
            continue;
        entity_s_def *od = (entity_s_def *)b->owner->def;
        if ( *(int *)&od->eclass->fixedsize )       // bbox brushes can't be reshaped
            continue;
        pick = b;
        break;
    }
    ...
    // The primitive functions stamp random_texture_stuff[0] onto every new face and
    // Brush_BuildWindings asserts that MaterialDef is valid; the headless selftest never
    // clicks a texture, so seed it from the picked brush's first face (what
    g_qeglobals.random_texture_stuff[0].mtl = pick->def->faces[0].mtldef[0];

    Select_Deselect( 1 );                            // exactly one brush selected
    Select_Brush( pick, 1, 0, 0 );

    if ( kind == 1 )
        Brush_MakeSidedCone( sides );                // N+1 faces
    else if ( kind == 2 )
        Brush_MakeSided( (int)pick->def, (unsigned)sides, 2 /*Z axis*/, 1 /*snap*/ ); // N+2 faces
    else
        Brush_MakeSidedSphere( sides );              // N*N faces (degenerate — see note)

    return pick->def->faceCount;
}

// patch gate: Patch_BrushToMesh (0x43ACC0) cylinder from the first editable world brush,
// optionally grown by Patch_InsertColumn.  outW0/outH0 = the grid after creation, outW1 =
// the width after the insert (0 if doInsert==0).  Returns 0 if no editable brush (N/A).
extern void Patch_BrushToMesh( char bCone, unsigned char bBevel,
                               unsigned char bEndcap, char bSquare );   // pmesh.cpp (0x43ACC0)
extern void Patch_InsertColumn( patchMesh_t *p, char flag );           // pmesh.cpp (0x443410)
extern "C" int Radiant_TestMakePatch( int doInsert, int *outW0, int *outH0, int *outW1 )
{
    *outW0 = *outH0 = *outW1 = 0;
    ...
    for ( selbrush_t *b = active_brushes.next; b && b != &active_brushes; b = b->next )
    {
        if ( b->patch || !b->def || b->def->faceCount <= 0 )
            continue;
        entity_s_def *od = (entity_s_def *)b->owner->def;
        if ( *(int *)&od->eclass->fixedsize )       // bbox brushes can't be patched
            continue;
        pick = b;
        break;
    }
    ...
    memcpy( &g_qeglobals.random_texture_stuff[0],
            &pick->def->faces[0].mtldef[0], sizeof( MaterialDef ) );

    Select_Deselect( 1 );
    Select_Brush( pick, 1, 0, 0 );

    Patch_BrushToMesh( 0, 0, 0, 0 );                 // cylinder: width=9, height=3

    // The new patch is now the single selected brush.  Read its grid.
    selbrush_t *sel = selected_brushes.next;
    if ( sel == &selected_brushes || !sel->def || !sel->def->patch )
        return 0;
    patchMesh_t *p = sel->def->patch;
    *outW0 = p->width;
    *outH0 = p->height;

    if ( doInsert )
    {
        Patch_InsertColumn( p, 1 );                  // add 2 columns at the grid end
        *outW1 = p->width;
    }
    return 1;
}

// simplepatch gate: Patch_GenericMesh (0x43b310, the Curve->Simple Patch Mesh backend).
// Seeds the layer-0 + layer-1 current-texture templates (the dialog inherits its texture
// from random_texture_stuff, NOT the source brush), then builds an nWidth x nHeight FLAT
// patch in the XY plane over the brush bounds and reads back the grid.
// Declared at file scope so it keeps C++ linkage (a local extern inside the extern "C"
// harness would mangle to a C name and fail to link).
selbrush_t *Patch_GenericMesh( int nWidth, int nHeight, int nOrientation,
                               char bDeleteSource, char bOverwrite );

extern "C" int Radiant_TestSimplePatchMesh( int nWidth, int nHeight, int *outW, int *outH )
{
    *outW = *outH = 0;
    ...
    memcpy( &g_qeglobals.random_texture_stuff[0],
            &pick->def->faces[0].mtldef[0], sizeof( MaterialDef ) );
    memcpy( &g_qeglobals.random_texture_stuff[1],
            &pick->def->faces[0].mtldef[0], sizeof( MaterialDef ) );

    Select_Deselect( 1 );
    Select_Brush( pick, 1, 0, 0 );

    // viewType 2 = ED_VIEW_XY (top); bDeleteSource=1, bOverwrite=0 (matches OnOK).
    selbrush_t *inst = Patch_GenericMesh( nWidth, nHeight, 2, 1, 0 );
    if ( !inst )
        return 0;
...
// patchrow gate harness: the InsertRow sibling of Radiant_TestMakePatch. Reuses the
// create path (doInsert=0 → 9×3 cylinder), then Patch_InsertRow (0x443830) to grow
// the grid along the OTHER axis (height 3 → 5), exercising the row-insert function
// the existing `patch` gate (which only drives Patch_InsertColumn) never covers.
// outW0/outH0 = grid after creation; outH1 = height after the row insert.
// Returns 1 on success, 0 if no editable world brush (N/A — pure-prefab map).
extern void Patch_InsertRow( patchMesh_t *p, char flag );              // pmesh.cpp (0x443830)
extern "C" int Radiant_TestMakePatchRow( int *outW0, int *outH0, int *outH1 )
{
    *outW0 = *outH0 = *outH1 = 0;

    int w0 = 0, h0 = 0, w1 = 0;
    if ( !Radiant_TestMakePatch( 0, &w0, &h0, &w1 ) )   // create 9×3 cylinder, no insert
        return 0;                                       // no editable world brush → N/A
    ...
    *outH0 = p->height;
    ...
    Patch_InsertRow( p, 1 );                            // add 2 rows at the grid end
    *outH1 = p->height;
    return 1;
}

// patchremove gate: both grid-SHRINK functions.  Patch_RemoveColumn drops the last 2
// columns (9 -> 7); then (RemoveRow's min-3 guard needs height > 3) InsertRow x2 grows
// 3 -> 7 and Patch_RemoveRow drops 7 -> 5.  A final 7x5 grid is reachable only if both
// removes ran.  outW1 = width after RemoveColumn, outH1 = height after grow+RemoveRow.
extern void Patch_RemoveColumn( patchMesh_t *p, char flag );           // pmesh.cpp (0x443C90)
extern void Patch_RemoveRow( patchMesh_t *p, char flag );              // pmesh.cpp (0x443B60)
extern "C" int Radiant_TestMakePatchRemove( int *outW0, int *outH0, int *outW1, int *outH1 )
{
    *outW0 = *outH0 = *outW1 = *outH1 = 0;
    ...
    *outH0 = p->height;                                 // 3
    ...
    Patch_RemoveColumn( p, 0 );                         // drop last 2 columns: 9 -> 7
    *outW1 = p->width;

    Patch_InsertRow( p, 1 );                            // 3 -> 5  (RemoveRow needs height > 3)
    Patch_InsertRow( p, 1 );                            // 5 -> 7
    Patch_RemoveRow( p, 0 );                            // drop last 2 rows: 7 -> 5
    *outH1 = p->height;
    return 1;
}

// patchinvert gate: Patch_ToggleInverted (the Curve->Negative core).  outFlipped = 1 iff
// rows 0 and height-1 swapped Z for every column (a result no other edit produces);
// outVerBump = the version delta (expect 1).
extern void Patch_ToggleInverted();                                    // pmesh.cpp (0x4465C0)
extern "C" int Radiant_TestMakePatchInvert( int *outW, int *outH, int *outFlipped, int *outVerBump )
{
    *outW = *outH = *outFlipped = *outVerBump = 0;
    ...
    *outH = p->height;                                  // 3
    ...
    float topZ[16], botZ[16];
    for ( int c = 0; c < W && c < 16; ++c )
    {
        topZ[c] = p->ctrl[c][0].xyz[2];
        botZ[c] = p->ctrl[c][H - 1].xyz[2];
    }
    const int verBefore = p->version;

    Patch_ToggleInverted();                             // vertical-flip the selected patch

    int flipped = ( H >= 2 );                           // a 1-row patch can't observably flip
    for ( int c = 0; c < W && c < 16; ++c )
        if ( p->ctrl[c][0].xyz[2] != botZ[c] || p->ctrl[c][H - 1].xyz[2] != topZ[c] )
        { flipped = 0; break; }
    *outFlipped = flipped;
    *outVerBump = (int)p->version - verBefore;
    return 1;
}

// patchtranspose gate: BOTH orientation branches of Patch_TransposeGrid (sub_449020).
// Transpose a 9x3 (width>height branch) then the resulting 3x9 (width<=height branch).
// After each pass the invariant is new[i][j] == old[newHeight-1-j][i] (transpose then
// vertical flip), checked element-exact into outMatch1/outMatch2.
...
extern "C" int Radiant_TestMakePatchTranspose( int *outW1, int *outH1, int *outMatch1,
                                               int *outW2, int *outH2, int *outMatch2 )
{
    *outW1 = *outH1 = *outMatch1 = *outW2 = *outH2 = *outMatch2 = 0;

    int w0 = 0, h0 = 0, w1c = 0;
    if ( !Radiant_TestMakePatch( 0, &w0, &h0, &w1c ) )   // create 9×3 cylinder, no insert
        return 0;                                        // no editable world brush → N/A
    selbrush_t *sel = selected_brushes.next;
    if ( sel == &selected_brushes || !sel->def || !sel->def->patch )
        return 0;
    patchMesh_t *p = sel->def->patch;
    if ( p->width != 9 || p->height != 3 )
        return 0;

    float g0[9][3][3];                                   // snapshot the original 9×3 grid
    for ( int c = 0; c < 9; ++c )
        for ( int r = 0; r < 3; ++r )
            for ( int k = 0; k < 3; ++k ) g0[c][r][k] = p->ctrl[c][r].xyz[k];

    Patch_Transpose();                                   // → 3×9 (width>height branch + flip)
    *outW1 = p->width; *outH1 = p->height;               // expect 3, 9
    int m1 = ( p->width == 3 && p->height == 9 );
    {
        const int H1 = p->height;                        // 9
        for ( int i = 0; i < p->width && m1; ++i )
            for ( int j = 0; j < H1 && m1; ++j )
                for ( int k = 0; k < 3; ++k )
                    if ( p->ctrl[i][j].xyz[k] != g0[H1 - 1 - j][i][k] ) { m1 = 0; break; }
    }
    *outMatch1 = m1;

    float g1[3][9][3];                                   // snapshot the 3×9 grid
    for ( int c = 0; c < 3; ++c )
        for ( int r = 0; r < 9; ++r )
            for ( int k = 0; k < 3; ++k ) g1[c][r][k] = p->ctrl[c][r].xyz[k];

    Patch_Transpose();                                   // → 9×3 (width<=height branch + flip)
    *outW2 = p->width; *outH2 = p->height;               // expect 9, 3
    int m2 = ( p->width == 9 && p->height == 3 );
    {
        const int H2 = p->height;                        // 3
        for ( int i = 0; i < p->width && m2; ++i )
            for ( int j = 0; j < H2 && m2; ++j )
                for ( int k = 0; k < 3; ++k )
                    if ( p->ctrl[i][j].xyz[k] != g1[H2 - 1 - j][i][k] ) { m2 = 0; break; }
    }
    *outMatch2 = m2;
    return 1;
}

// patchnaturalize gate: Patch_NaturalizeSelected (0x447FD0) + Patch_Naturalize2.
// Naturalize at 0.25 then 0.5: st is proportional to 1/scale, so every control point's
// S/T must HALVE - a ratio independent of geometry and texture size.  outVerBump = 2.
extern void Patch_NaturalizeSelected( bool unk, bool cap, float x, float y );  // pmesh.cpp (0x447FD0)
extern "C" int Radiant_TestMakePatchNaturalize( int *outW, int *outH, int *outRatioOk, int *outVerBump )
{
    *outW = *outH = *outRatioOk = *outVerBump = 0;
    ...
    if ( sel == &selected_brushes || !sel->def || !sel->def->patch )
        return 0;
    patchMesh_t *p = sel->def->patch;
    *outW = p->width; *outH = p->height;                 // 9, 3
    const int W = p->width, H = p->height;

    const int verBefore = p->version;
    Patch_NaturalizeSelected( 0, 0, 0.25f, 0.25f );      // scale A
    float stA[16][16][2];
    for ( int i = 0; i < W && i < 16; ++i )
        for ( int j = 0; j < H && j < 16; ++j )
        { stA[i][j][0] = p->ctrl[i][j].texCoord.st[0];
          stA[i][j][1] = p->ctrl[i][j].texCoord.st[1]; }

    Patch_NaturalizeSelected( 0, 0, 0.5f, 0.5f );        // scale B = 2*A → S/T halves
    int ratioOk = 1;
    for ( int i = 0; i < W && i < 16 && ratioOk; ++i )
        for ( int j = 0; j < H && j < 16 && ratioOk; ++j )
            for ( int k = 0; k < 2; ++k )
            {
                float expected = stA[i][j][k] * 0.5f;
                float got      = p->ctrl[i][j].texCoord.st[k];
                float d        = got - expected; if ( d < 0 ) d = -d;
                float e        = expected < 0 ? -expected : expected;
                if ( d > 1e-3f + 1e-3f * e ) { ratioOk = 0; break; }
            }
    *outRatioOk = ratioOk;
    *outVerBump = (int)p->version - verBefore;           // expect 2 (two naturalize calls)
    return 1;
}

// patchdisperse gate: Curve->Redisperse Cols/Rows.  Perturb an interior control point off
// the chord, disperse, and verify the cumulative spacing formula holds along each axis.
extern void Patch_DisperseColumns();   // pmesh.cpp (0x4443A0)
extern void Patch_DisperseRows();      // pmesh.cpp (0x444200)
static int ST_PatchColLinear( patchMesh_t *p )
{
    for ( int r = 0; r < p->height; ++r )
    {
        if ( p->width <= 1 ) continue;
        float ex[16][3], step[3];
        for ( int k = 0; k < 3; ++k )
        {
            ex[0][k] = p->ctrl[0][r].xyz[k];
            step[k]  = ( p->ctrl[p->width - 1][r].xyz[k] - p->ctrl[0][r].xyz[k] ) / (float)( p->width - 1 );
        }
        for ( int j = 1; j < p->width; ++j )
            for ( int k = 0; k < 3; ++k ) ex[j][k] = ex[j - 1][k] + step[k];
        for ( int j = 0; j < p->width; ++j )
            for ( int k = 0; k < 3; ++k )
            {
                float d = p->ctrl[j][r].xyz[k] - ex[j][k]; if ( d < 0 ) d = -d;
                float e = ex[j][k] < 0 ? -ex[j][k] : ex[j][k];
                if ( d > 0.05f + 1e-4f * e ) return 0;
            }
    }
    return 1;
}
static int ST_PatchRowLinear( patchMesh_t *p )
{
    for ( int c = 0; c < p->width; ++c )
    {
        ...
        for ( int k = 0; k < 3; ++k )
        {
            ex[0][k] = p->ctrl[c][0].xyz[k];
            step[k]  = ( p->ctrl[c][p->height - 1].xyz[k] - p->ctrl[c][0].xyz[k] ) / (float)( p->height - 1 );
        }
        ...
        for ( int j = 0; j < p->height; ++j )
            for ( int k = 0; k < 3; ++k )
            {
                float d = p->ctrl[c][j].xyz[k] - ex[j][k]; if ( d < 0 ) d = -d;
                float e = ex[j][k] < 0 ? -ex[j][k] : ex[j][k];
                if ( d > 0.05f + 1e-4f * e ) return 0;
            }
    }
...
extern "C" int Radiant_TestDispersePatch( int *outW, int *outH, int *outColLinear,
                                          int *outRowLinear, int *outVerBumped )
{
    *outW = *outH = *outColLinear = *outRowLinear = *outVerBumped = 0;
    ...
    if ( sel == &selected_brushes || !sel->def || !sel->def->patch )
        return 0;
    patchMesh_t *p = sel->def->patch;
    *outW = p->width; *outH = p->height;

    if ( p->width >= 3 && p->height >= 3 )               // perturb an interior point off the chord
        p->ctrl[2][1].xyz[2] += 500.0f;
    const int verBefore = p->version;

    Patch_DisperseColumns();
    *outColLinear = ST_PatchColLinear( p );              // each row now linear across columns
    Patch_DisperseRows();
    *outRowLinear = ST_PatchRowLinear( p );              // each column now linear across rows
    *outVerBumped = ( p->version > verBefore ) ? 1 : 0;
    return 1;
}

// patchinvtex gate harness: Curve→Negative Texture X/Y (Patch_InvertTexture 0x446680).
// Make a 9x3 cylinder (naturalized linear st), snapshot the current-layer st, then for each
// axis: invert once (st must CHANGE — the linear st is mirrored) and invert again (st must
// be RESTORED — the op is an involution).  Verifies BOTH the columns (axis 1) and rows
// (axis 0) branches.  outChanged*/outRestored* = 1 on success.  Returns 1, or 0 if N/A.
extern void Patch_InvertTexture( char axis );   // pmesh.cpp (0x446680)
static int ST_PatchStDiffers( patchMesh_t *p, float st0[16][16][2], int layer )
{
    for ( int c = 0; c < p->width && c < 16; ++c )
        for ( int r = 0; r < p->height && r < 16; ++r )
            for ( int k = 0; k < 2; ++k )
            {
                float cur = p->ctrl[c][r].texCoord.st[2 * layer + k];
                float d   = cur - st0[c][r][k]; if ( d < 0 ) d = -d;
                if ( d > 1e-5f ) return 1;
            }
    return 0;
}
extern "C" int Radiant_TestInvertTexture( int *outW, int *outH, int *outChangedY, int *outRestoredY,
                                          int *outChangedX, int *outRestoredX )
{
    *outW = *outH = *outChangedY = *outRestoredY = *outChangedX = *outRestoredX = 0;
    ...
    *outW = p->width; *outH = p->height;
    ...
    float st0[16][16][2];                                // snapshot current-layer st
    for ( int c = 0; c < p->width && c < 16; ++c )
        for ( int r = 0; r < p->height && r < 16; ++r )
        { st0[c][r][0] = p->ctrl[c][r].texCoord.st[2 * layer];
          st0[c][r][1] = p->ctrl[c][r].texCoord.st[2 * layer + 1]; }

    Patch_InvertTexture( 1 );                            // mirror columns
    *outChangedY  = ST_PatchStDiffers( p, st0, layer );
    Patch_InvertTexture( 1 );
    *outRestoredY = !ST_PatchStDiffers( p, st0, layer );

    Patch_InvertTexture( 0 );                            // mirror rows
    *outChangedX  = ST_PatchStDiffers( p, st0, layer );
    Patch_InvertTexture( 0 );
    *outRestoredX = !ST_PatchStDiffers( p, st0, layer );
    return 1;
}

// patchscale gate harness: Patch_Scale (0x4427D0), reached by Select_Scale's patch branch
// (was a FATAL stub).  Make a 9x3 cylinder, scale every control point by 2x about the origin,
// and verify ctrl[2][1].xyz == old*2 (the (xyz-mid)*scale+mid formula with mid=0).
extern void Patch_Scale( patchMesh_t *p, const float *mid, const float *scale, char doRebuild );  // pmesh.cpp
extern "C" int Radiant_TestPatchScale( int *outW, int *outH, int *outScaledOk )
{
    *outW = *outH = *outScaledOk = 0;
    ...
    *outW = p->width; *outH = p->height;
    ...
    g_qeglobals.d_select_mode = sel_brush;               // scale ALL points (not the move-point subset)
    const float old0 = p->ctrl[2][1].xyz[0];
    const float old1 = p->ctrl[2][1].xyz[1];
    const float old2 = p->ctrl[2][1].xyz[2];
    const float mid[3]   = { 0.0f, 0.0f, 0.0f };
    const float scale[3] = { 2.0f, 2.0f, 2.0f };

    Patch_Scale( p, mid, scale, 1 );

    int ok = 1;
    const float exp[3] = { old0 * 2.0f, old1 * 2.0f, old2 * 2.0f };
    for ( int k = 0; k < 3; ++k )
    {
        float d = p->ctrl[2][1].xyz[k] - exp[k]; if ( d < 0 ) d = -d;
        float e = exp[k] < 0 ? -exp[k] : exp[k];
        if ( d > 0.05f + 1e-4f * e ) ok = 0;
    }
    *outScaledOk = ok;
    return 1;
}

// patchmove gate harness: Patch_Move (0x441dd0), reached by Brush_Move's patch branch
// (MoveSelection routes a dragged patch here — was a FATAL stub, so dragging a patch crashed).
// Make a 9x3 cylinder, snapshot every control point's xyz, translate by a known delta
// {64,32,16}, and verify EVERY control point shifted by exactly that delta AND the curveDef
// was regenerated (non-null) with version bumped.  outMovedOk / outRegen = 1 on success.
extern curvePatchDef_t *Patch_Move( const float *move, patchMesh_t *p );  // pmesh.cpp (0x441dd0)
extern "C" int Radiant_TestPatchMove( int *outW, int *outH, int *outMovedOk, int *outRegen )
{
    *outW = *outH = *outMovedOk = *outRegen = 0;
    ...
    *outW = p->width; *outH = p->height;
    ...
    float xyz0[16][16][3];                               // snapshot every control point
    for ( int c = 0; c < p->width && c < 16; ++c )
        for ( int r = 0; r < p->height && r < 16; ++r )
            for ( int k = 0; k < 3; ++k )
                xyz0[c][r][k] = p->ctrl[c][r].xyz[k];

    const short          verBefore = p->version;
    const float          move[3]   = { 64.0f, 32.0f, 16.0f };

    curvePatchDef_t *cd = Patch_Move( move, p );
    ...
    for ( int c = 0; c < p->width && c < 16; ++c )
        for ( int r = 0; r < p->height && r < 16; ++r )
            for ( int k = 0; k < 3; ++k )
            {
                float exp = xyz0[c][r][k] + move[k];
                float d   = p->ctrl[c][r].xyz[k] - exp; if ( d < 0 ) d = -d;
                float e   = exp < 0 ? -exp : exp;
                if ( d > 0.01f + 1e-4f * e ) moved = 0;
            }
    *outMovedOk = moved;
    // curveDef regenerated (non-null), returned, and version advanced.
    *outRegen   = ( p->curveDef != nullptr && cd == p->curveDef && p->version != verBefore ) ? 1 : 0;
    return 1;
}

// patchshifttex gate harness: Patch_ShiftTexture (0x446170), reached by Brush_ShiftTexture's
// patch branch (was a FATAL stub).  Make a 9x3 cylinder, shift the layer st by (5.0, 0.25),
// and verify a control point's st advanced by (0.5, 0.25) — exercising the |shift|>=1 ÷10 clamp
// (5.0→0.5) and the direct add (0.25).
extern void Patch_ShiftTexture( patchMesh_t *p, float s, float t );  // pmesh.cpp (0x446170)
extern "C" int Radiant_TestPatchShiftTex( int *outW, int *outH, int *outShiftedOk )
{
    *outW = *outH = *outShiftedOk = 0;

    int w0 = 0, h0 = 0, w1 = 0;
    if ( !Radiant_TestMakePatch( 0, &w0, &h0, &w1 ) )    // create 9×3 cylinder
        return 0;
    selbrush_t *sel = selected_brushes.next;
    if ( sel == &selected_brushes || !sel->def || !sel->def->patch )
        return 0;
    ...
    *outW = p->width; *outH = p->height;
    ...
    g_qeglobals.d_select_mode = sel_brush;               // shift ALL control points
    const int layer = g_qeglobals.current_edit_layer;
    const float s0 = p->ctrl[2][1].texCoord.st[2 * layer];
    const float t0 = p->ctrl[2][1].texCoord.st[2 * layer + 1];

    Patch_ShiftTexture( p, 5.0f, 0.25f );                // 5.0 → /10 = 0.5 ; 0.25 stays

    float ds = p->ctrl[2][1].texCoord.st[2 * layer]     - ( s0 + 0.5f );  if ( ds < 0 ) ds = -ds;
    float dt = p->ctrl[2][1].texCoord.st[2 * layer + 1] - ( t0 + 0.25f ); if ( dt < 0 ) dt = -dt;
    *outShiftedOk = ( ds <= 1e-4f && dt <= 1e-4f ) ? 1 : 0;
    return 1;
}

// patchrottex gate harness: Patch_RotateTexture (0x445B80), reached by Brush_RotateTexture's
// patch branch (was a FATAL stub).  Make a 9x3 cylinder, rotate the layer st by 90° and verify
// a control point's st became (-t0, s0) — a 90° rotation about the texel origin with the headless
// 512x512 default texel size (texW==texH, so the /texW,*texH factors cancel).
extern void Patch_RotateTexture( patchMesh_t *p, float deg );  // pmesh.cpp (0x445B80)
extern "C" int Radiant_TestPatchRotateTex( int *outW, int *outH, int *outRotatedOk )
{
    *outW = *outH = *outRotatedOk = 0;
    ...
    *outW = p->width; *outH = p->height;
    ...
    g_qeglobals.d_select_mode = sel_brush;               // rotate ALL control points
    const int layer = g_qeglobals.current_edit_layer;
    const float s0 = p->ctrl[2][1].texCoord.st[2 * layer];
    const float t0 = p->ctrl[2][1].texCoord.st[2 * layer + 1];

    Patch_RotateTexture( p, 90.0f );                     // 90° about texel origin → (s,t)→(-t,s)

    float ds = p->ctrl[2][1].texCoord.st[2 * layer]     - ( -t0 ); if ( ds < 0 ) ds = -ds;
    float dt = p->ctrl[2][1].texCoord.st[2 * layer + 1] - (  s0 ); if ( dt < 0 ) dt = -dt;
    float es = t0 < 0 ? -t0 : t0;
    float et = s0 < 0 ? -s0 : s0;
    *outRotatedOk = ( ds <= 1e-3f + 1e-3f * es && dt <= 1e-3f + 1e-3f * et ) ? 1 : 0;
    return 1;
}

// patchscaletex gate harness: Patch_ScaleTexture (0x445F80), reached by Brush_ScaleTexture's
// patch branch (was a FATAL stub).  Make a 9x3 cylinder, scale the layer st by (2,3), and verify
// a control point's st became (s0*2, t0*3).
extern void Patch_ScaleTexture( patchMesh_t *p, float s, float t );  // pmesh.cpp (0x445F80)
extern "C" int Radiant_TestPatchScaleTex( int *outW, int *outH, int *outScaledOk )
{
    *outW = *outH = *outScaledOk = 0;
    ...
    *outW = p->width; *outH = p->height;
    ...
    g_qeglobals.d_select_mode = sel_brush;               // scale ALL control points
    const int layer = g_qeglobals.current_edit_layer;
    const float s0 = p->ctrl[2][1].texCoord.st[2 * layer];
    const float t0 = p->ctrl[2][1].texCoord.st[2 * layer + 1];

    Patch_ScaleTexture( p, 2.0f, 3.0f );                 // st0 *= 2, st1 *= 3

    float ds = p->ctrl[2][1].texCoord.st[2 * layer]     - ( s0 * 2.0f ); if ( ds < 0 ) ds = -ds;
    float dt = p->ctrl[2][1].texCoord.st[2 * layer + 1] - ( t0 * 3.0f ); if ( dt < 0 ) dt = -dt;
    float es = s0 < 0 ? -s0 : s0;
    float et = t0 < 0 ? -t0 : t0;
    *outScaledOk = ( ds <= 1e-3f + 1e-3f * es && dt <= 1e-3f + 1e-3f * et ) ? 1 : 0;
    return 1;
}

// patchfittex gate harness: Patch_FitTexturing (0x447600), reached by Brush_FitTextureFaces'
// patch branch (the Surface Inspector "Fit" on a patch — was a FATAL stub).  Make a 9x3 cylinder,
// Fit, and verify each control point's st became its normalized grid position
// (s = col/(width-1), t = -row/(height-1)).
extern void Patch_FitTexturing( patchMesh_t *p );  // pmesh.cpp (0x447600)
extern "C" int Radiant_TestPatchFitTex( int *outW, int *outH, int *outFitOk )
{
    *outW = *outH = *outFitOk = 0;
    ...
    *outW = p->width; *outH = p->height;
    ...
    Patch_FitTexturing( p );                             // st = normalized grid position

    int ok = 1;
    for ( int col = 0; col < p->width && ok; ++col )
        for ( int row = 0; row < p->height && ok; ++row )
        {
            float es =  (float)col / (float)( p->width  - 1 );
            float et = -(float)row / (float)( p->height - 1 );
            float ds = p->ctrl[col][row].texCoord.st[2 * layer]     - es; if ( ds < 0 ) ds = -ds;
            float dt = p->ctrl[col][row].texCoord.st[2 * layer + 1] - et; if ( dt < 0 ) dt = -dt;
            if ( ds > 1e-4f || dt > 1e-4f )
                ok = 0;
        }
    *outFitOk = ok;
    return 1;
...
// patchapplymat gate: Patch_ApplyMatrix (0x441E70), the patch branch of
// Select_ApplyMatrix.  180-degrees about Z centred at ctrl[0][0], snap off: every control
// point must reflect through that centre, (x,y,z) -> (2*ox - x, 2*oy - y, z).
extern "C" int Radiant_TestPatchApplyMat( int *outW, int *outH, int *outMovedOk, int *outPivotOk )
{
    *outW = *outH = *outMovedOk = *outPivotOk = 0;
    ...
    *outW = p->width; *outH = p->height;
    ...
    g_qeglobals.d_select_mode = sel_brush;               // transform ALL control points
    g_bPatchBendMode = 0;

    // 180° about Z, centred at control point [0][0].
    orientation_t orient;
    orient.origin[0] = p->ctrl[0][0].xyz[0];
    orient.origin[1] = p->ctrl[0][0].xyz[1];
    orient.origin[2] = p->ctrl[0][0].xyz[2];
    orient.axis[0][0] = -1.0f; orient.axis[0][1] =  0.0f; orient.axis[0][2] = 0.0f;
    orient.axis[1][0] =  0.0f; orient.axis[1][1] = -1.0f; orient.axis[1][2] = 0.0f;
    orient.axis[2][0] =  0.0f; orient.axis[2][1] =  0.0f; orient.axis[2][2] = 1.0f;

    const float ox = orient.origin[0], oy = orient.origin[1];
    float bx = p->ctrl[2][1].xyz[0], by = p->ctrl[2][1].xyz[1], bz = p->ctrl[2][1].xyz[2];

    Patch_ApplyMatrix( &orient, p, 0 );                  // snap off

    // moved control point reflected through the pivot
    float dx = p->ctrl[2][1].xyz[0] - ( 2.0f * ox - bx ); if ( dx < 0 ) dx = -dx;
    float dy = p->ctrl[2][1].xyz[1] - ( 2.0f * oy - by ); if ( dy < 0 ) dy = -dy;
    float dz = p->ctrl[2][1].xyz[2] - bz;                 if ( dz < 0 ) dz = -dz;
    *outMovedOk = ( dx <= 1e-2f && dy <= 1e-2f && dz <= 1e-2f ) ? 1 : 0;

    // the pivot control point [0][0] stays fixed (rel == 0)
    float px = p->ctrl[0][0].xyz[0] - ox; if ( px < 0 ) px = -px;
    float py = p->ctrl[0][0].xyz[1] - oy; if ( py < 0 ) py = -py;
    *outPivotOk = ( px <= 1e-2f && py <= 1e-2f ) ? 1 : 0;
    return 1;
}

// samplesize gate: Brush_SetSampleSize (0x48F800) brush pass -> sub_477080 (0x477080).
// Reads back face[0]'s lightmap-channel texdef scale, which sub_477080 sets to
// sampleSize * material w/h; the headless layered material is 512x512, so the result is
// sampleSize*512.  Returns -1 if no editable brush is selected.
void Brush_SetSampleSize( int size );   // defined below (0x48F800)
extern "C" float Radiant_TestSampleSize( int size )
{
    selbrush_t *sel = selected_brushes.next;
    if ( sel == &selected_brushes || !sel->def || sel->def->faceCount < 1 )
        return -1.0f;
    Brush_SetSampleSize( size );                 // brush pass (no face selected) -> sub_477080
    sel = selected_brushes.next;
    if ( sel == &selected_brushes || !sel->def || sel->def->faceCount < 1 )
        return -1.0f;
    MaterialDef *md = &sel->def->faces[0].mtldef[1];   // lightmap channel
    return md->mat_texDef.size[ 7 * g_qeglobals.current_edit_layer ];
}

// texmapping gate: Brush_SetTextureMapping (0x48F4F0) brush pass -> sub_477020 (0x477020).
// sub_477020 copies the texdef into every face's current-layer mat_texDef (via sub_4767E0,
// a verbatim memcpy), so face[0]'s mat_texDef.size[0] must come back as 13.0 (-1 = none).
void Brush_SetTextureMapping( texdef_sub_t *a2 );   // defined below (0x48F4F0)
extern "C" float Radiant_TestTexMapping()
{
    selbrush_t *sel = selected_brushes.next;
    if ( sel == &selected_brushes || !sel->def || sel->def->faceCount < 1 )
        return -1.0f;
    ...
    td.size[0] = 13.0f;                          // distinctive scale
    Brush_SetTextureMapping( &td );              // brush pass (no face selected) -> sub_477020

    sel = selected_brushes.next;
    if ( sel == &selected_brushes || !sel->def || sel->def->faceCount < 1 )
        return -1.0f;
    MaterialDef *md = &sel->def->faces[0].mtldef[ g_qeglobals.current_edit_layer ];
    return md->mat_texDef.size[0];               // expect 13.0 (verbatim copy)
}

// hollow gate: CSG_MakeHollow (0x47D3C0).  Each face of the single selected brush is inset
// by gridScale and split, keeping the front wall slab and freeing the original, so the
// total brush count grows by numSlabs-1 (5 for a solid cube).
void CSG_MakeHollow();   // csg.cpp (0x47D3C0)
extern "C" int Radiant_TestHollow( int *outSelBefore, int *outDelta )
{
    *outSelBefore = *outDelta = 0;

    int selBefore = 0;
    for ( selbrush_t *b = selected_brushes.next; b != &selected_brushes; b = b->next ) ++selBefore;
    *outSelBefore = selBefore;
    if ( selBefore != 1 )           // expect exactly the created cube
        return 0;

    int totalBefore = selBefore;
    for ( selbrush_t *b = active_brushes.next; b != &active_brushes; b = b->next ) ++totalBefore;

    CSG_MakeHollow();

    int totalAfter = 0;
    for ( selbrush_t *b = selected_brushes.next; b != &selected_brushes; b = b->next ) ++totalAfter;
    for ( selbrush_t *b = active_brushes.next;   b != &active_brushes;   b = b->next ) ++totalAfter;

    *outDelta = totalAfter - totalBefore;
    return 1;
}

// nsided gate: Brush_MakeSided (0x4731E0) reshapes the selected brush IN-PLACE into an
// N-sided cylinder (N side faces + top/bottom caps).  Verifies faceCount == sides+2 and
// that every face built a valid winding.
extern void Brush_MakeSided( int brushDef, unsigned int sides, int axis, char snap );  // brush.cpp 0x4731E0
extern "C" int Radiant_TestMakeSided( int sides, int *outFaceCount, int *outWindingsOk,
                                      int *outGeomOk )
{
    *outFaceCount = *outWindingsOk = 0;
    if ( outGeomOk ) *outGeomOk = 0;
    selbrush_t *sel = selected_brushes.next;
    if ( sel == &selected_brushes || !sel->def )
        return 0;
    brush_t *def = sel->def;

    // Derive the EXACT resulting hexagon AABB.  Brush_MakeSided (0x4731E0, axis=Z) takes
    // center[k] = (mins[k]+maxs[k])*0.5 and radius = max(1.0, max over the TWO non-axis
    // dims of (maxs[k]-mins[k])*0.5), lays the caps on the axis at the original
    // mins.z/maxs.z, and puts the side vertices at center.xy + radius*(cos t, sin t).  With
    // sides=6 the first vertex is at t=0 and t=180 is exact, so the X-extent is exactly
    // [center.x-radius, center.x+radius] and Z is unchanged (snap=0, no floor-snap).
    float preMins[3] = { def->mins[0], def->mins[1], def->mins[2] };
    float preMaxs[3] = { def->maxs[0], def->maxs[1], def->maxs[2] };
    float center[3];
    float radius = 1.0f;
    for ( int k = 0; k < 3; ++k )
    {
        center[k] = ( preMins[k] + preMaxs[k] ) * 0.5f;
        if ( k != 2 )   // axis = Z (2): radius from the two non-axis dims
        {
            float half = ( preMaxs[k] - preMins[k] ) * 0.5f;
            if ( radius < half ) radius = half;
        }
    }

    Brush_MakeSided( (int)(intptr_t)def, (unsigned int)sides, 2, 0 );   // axis Z, no snap

    *outFaceCount = def->faceCount;
    int wok = ( def->faceCount > 0 ) ? 1 : 0;
    for ( int i = 0; i < def->faceCount; ++i )
        if ( !def->faces[i].w ) { wok = 0; break; }
    *outWindingsOk = wok;

    // EXACT GEOMETRY (gate-hardening): the resulting hexagon's AABB must be the derived box.
    //   X: [center.x − radius, center.x + radius]  (vertices at θ=0° / 180°, cos exact ±1)
    //   Z: unchanged [preMins.z, preMaxs.z]        (caps lie on the axis planes)
    // Both are float-exact; a small magnitude-scaled epsilon absorbs ULP wobble.
    if ( outGeomOk )
    {
        const float expMinX = center[0] - radius;
        const float expMaxX = center[0] + radius;
        const float tolX = (float)( ( fabs( center[0] ) + fabs( radius ) ) * 1e-5 ) + 0.01f;
        const float tolZ = (float)( ( fabs( preMins[2] ) + fabs( preMaxs[2] ) ) * 1e-5 ) + 0.01f;
        bool geom = fabs( def->mins[0] - expMinX )   <= tolX
                 && fabs( def->maxs[0] - expMaxX )   <= tolX
                 && fabs( def->mins[2] - preMins[2] ) <= tolZ
                 && fabs( def->maxs[2] - preMaxs[2] ) <= tolZ;
        *outGeomOk = geom ? 1 : 0;
    }
    return 1;
}

// floortrace gate: Trace_AllDirectionsIfFailed (0x48DAA0), the drop-to-floor ray.  Drops
// the created box out of the selection (so it is the world being dropped onto) and traces
// straight down from 100 units above its centre with contents 4610 - the exact mask
// CMainFrame::OnDropSelected passes (0x0002 skip the selected list, 0x0200 exclude
// fixedsize point entities, 0x1000 allow flag-0x20 brushes).  The hit must be the box's
// top face at distance 100 with normal (0,0,1).
extern edTrace_t *Trace_AllDirectionsIfFailed( float *cam_origin, edTrace_t *trace_result,
                                               float *dir, int contents );  // above (0x48DAA0)
// NOTE: contents bits SUPPRESS a list (Test_Ray walks active when (c&1)==0 and selected
// when (c&2)==0), and sub_48D460 skips every BRUSHFLAG_SELECTED brush - so contents==1
// can never hit anything.
extern void Select_Deselect( int a1 );

extern "C" int Radiant_TestFloorTrace( int *outHit, float *outDist, float *outNormalZ )
{
    *outHit = 0; *outDist = -1.0f; *outNormalZ = 0.0f;
    selbrush_t *sel = selected_brushes.next;
    if ( sel == &selected_brushes || !sel->def || sel->def->faceCount < 1 )
        return 0;
    brush_t *def = sel->def;

    float start[3] = { ( def->mins[0] + def->maxs[0] ) * 0.5f,
                       ( def->mins[1] + def->maxs[1] ) * 0.5f,
                       def->maxs[2] + 100.0f };
    float dir[3] = { 0.0f, 0.0f, -1.0f };

    Select_Deselect( 1 );          // box -> active list, BRUSHFLAG_SELECTED cleared

    edTrace_t tr;
    Trace_AllDirectionsIfFailed( start, &tr, dir, 4610 );   // == CMainFrame::OnDropSelected

    *outHit     = ( tr.brush != nullptr ) ? 1 : 0;
    *outDist    = tr.dist;
    *outNormalZ = tr.normal[2];
    // Identity check: the nearest thing 100u below the ray start must be the box we made,
    // not some pre-existing mp_test brush that happens to sit in the gap.
    return ( tr.brush == sel ) ? 1 : 2;
}

// autocaulk gate: Brush_AutoCaulk (0x47E0F0), the wired Selection->CSG->Auto Caulk command.
// AutoCaulk only retextures faces - it never adds or removes brushes - so the active +
// selected list count must be unchanged across the call.
// HEADLESS LIMITATION: textures are not GPU-realized in the selftest, so every layer's
// qtexture_s.in_use is 0, MaterialDef_08 leaves dword_181F51C at its preset 128, and the
// per-face `if (!dword_181F51C)` caulk body is never entered.  What this gate DOES verify
// is that the command runs over a multi-brush selection without crashing (exercising the
// MaterialDef_02/_08 realize call) and leaves the brush list intact and walkable.
extern void Brush_AutoCaulk();   // csg.cpp (0x47E0F0)
extern "C" int Radiant_TestAutoCaulk( int *outSelCount, int *outListDelta, int *outIntact )
{
    *outSelCount = *outListDelta = *outIntact = 0;

    // Promote the most-recently created active brush back into the selection so AutoCaulk
    // runs over MORE THAN ONE selected brush (the runner created A, deselected it, created B).
    if ( active_brushes.next != &active_brushes )
        Select_Brush( active_brushes.next, 0, 0, 0 );

    int selCount = 0;
    for ( selbrush_t *b = selected_brushes.next; b != &selected_brushes; b = b->next ) ++selCount;
    *outSelCount = selCount;

    int totalBefore = 0;
    for ( selbrush_t *b = selected_brushes.next; b != &selected_brushes; b = b->next ) ++totalBefore;
    for ( selbrush_t *b = active_brushes.next;   b != &active_brushes;   b = b->next ) ++totalBefore;

    Brush_AutoCaulk();   // the wired command: realize each selected face's material, caulk hidden ones

    int totalAfter = 0;
    int intact = 1;
    for ( selbrush_t *b = selected_brushes.next; b != &selected_brushes; b = b->next )
    { if ( !b->def ) { intact = 0; break; } ++totalAfter; }
    for ( selbrush_t *b = active_brushes.next;   b != &active_brushes;   b = b->next )
    { if ( !b->def ) { intact = 0; break; } ++totalAfter; }

    *outListDelta = totalAfter - totalBefore;   // AutoCaulk retextures faces only => expect 0
    *outIntact    = intact;
    return 1;
}

// patchflip gate: the end-to-end "Brush -> Flip X" op on a selected PATCH -
// Select_FlipAxis(0) -> Select_ApplyMatrix_SelectedBrushes -> Select_ApplyMatrix -> the
// patch branch Patch_ApplyMatrix (0x441E70).  Flip mirrors across the axis plane through
// Select_GetMid with bSnap=1, so each control point reflects on X and then snaps to the
// nearest half-unit.  This is the only gate that drives a SELECTION-level transform
// through to a patch (it covers the snap path patchapplymat skips).
void Select_FlipAxis( int axis );   // defined below (0x48FD50)
extern "C" int Radiant_TestPatchFlip( int *outW, int *outH, int *outReflOk )
{
    *outW = *outH = *outReflOk = 0;

    int w0 = 0, h0 = 0, w1 = 0;
    if ( !Radiant_TestMakePatch( 0, &w0, &h0, &w1 ) )    // create 9×3 cylinder (left selected)
        return 0;
    selbrush_t *sel = selected_brushes.next;
    if ( sel == &selected_brushes || !sel->def || !sel->def->patch )
        return 0;
    ...
    *outW = p->width; *outH = p->height;
    ...
    g_qeglobals.d_select_mode = sel_brush;               // flip ALL control points
    g_bPatchBendMode = 0;

    float mid[3];
    Select_GetMid( mid );                                // the same pivot Select_FlipAxis recomputes
    float bx = p->ctrl[2][1].xyz[0];
    float by = p->ctrl[2][1].xyz[1];
    float bz = p->ctrl[2][1].xyz[2];

    Select_FlipAxis( 0 );                                // mirror across the X plane (snap on)

    // expected = the snapped reflection (snap(c) = floor(0.5 + 2c) * 0.5, matching Patch_ApplyMatrix)
    float ex = (float)floor( 0.5 + 2.0 * ( 2.0f * mid[0] - bx ) ) * 0.5f;
    float ey = (float)floor( 0.5 + 2.0 * by ) * 0.5f;
    float ez = (float)floor( 0.5 + 2.0 * bz ) * 0.5f;

    float dx = p->ctrl[2][1].xyz[0] - ex; if ( dx < 0 ) dx = -dx;
    float dy = p->ctrl[2][1].xyz[1] - ey; if ( dy < 0 ) dy = -dy;
    float dz = p->ctrl[2][1].xyz[2] - ez; if ( dz < 0 ) dz = -dz;
    *outReflOk = ( dx <= 1e-2f && dy <= 1e-2f && dz <= 1e-2f ) ? 1 : 0;
    return 1;
}

// patchfliptex gate: the end-to-end "Brush -> Flip Texture X" op on a selected PATCH ->
// Brush_FlipTexture's patch branch -> PMESH_37 (Patch_FlipTexture 0x445e30).  Also covers
// the instance-vs-def caller fix (the binary passes v3->patch->def).  st[S] must become
// 1 - s0 with T unchanged, and the curveDef must rebuild.
void Brush_FlipTexture( int a1 );                                   // defined below (0x492280)
extern "C" int Radiant_TestPatchFlipTex( int *outW, int *outH, int *outFlippedOk, int *outRegen )
{
    *outW = *outH = *outFlippedOk = *outRegen = 0;
    ...
    *outW = p->width; *outH = p->height;
    ...
    g_qeglobals.d_select_mode = sel_brush;               // flip ALL control points (not curvepoint)
    g_bPatchBendMode = 0;
    const int   layer = g_qeglobals.current_edit_layer;
    const float s0    = p->ctrl[2][1].texCoord.st[2 * layer];      // S of the current layer
    const float t0    = p->ctrl[2][1].texCoord.st[2 * layer + 1];  // T must be UNCHANGED (axis 0)
    curvePatchDef_t *cdBefore = p->curveDef;
    short verBefore = p->version;

    Brush_FlipTexture( 0 );                              // axis 0 = flip X (S) → st[S] = 1 - st[S]

    float ds = p->ctrl[2][1].texCoord.st[2 * layer]     - ( 1.0f - s0 ); if ( ds < 0 ) ds = -ds;
    float dt = p->ctrl[2][1].texCoord.st[2 * layer + 1] - t0;            if ( dt < 0 ) dt = -dt;
    *outFlippedOk = ( ds <= 1e-4f && dt <= 1e-4f ) ? 1 : 0;
    // PMESH_37 frees + regenerates the curveDef and bumps version.  (Don't compare the
    // curveDef POINTER — the allocator routinely hands back the just-freed block, so a
    // fresh-but-same-address curveDef is expected; check non-null + the version bump.)
    (void)cdBefore;
    *outRegen = ( p->curveDef != nullptr && p->version != verBefore ) ? 1 : 0;
    return 1;
}

// patchturnedge gate harness: the terrain-edge-turn picker chain — Patch_TurnEdge
// (0x44b4c0) → PMESH_51 (0x44acc0) → PMESH_RaySegPick (0x44ab10); all were FATAL/parked.
// Build a FLAT terrain grid (z=0, 64-unit cells), aim a straight-down ray through the
// centre of cell (0,0), and verify the picker toggles exactly that cell's turned_edge.
extern void Patch_TurnEdge( int org, int dir );                                 // pmesh.cpp 0x44b4c0
extern curvePatchDef_t *Patch_GenericMesh2( patchMesh_t *p, int layer, int colMap, int rowMap ); // pmesh.cpp 0x438b60
extern "C" int Radiant_TestPatchTurnEdge( int *outBefore, int *outToggled, int *outOnlyOne )
{
    *outBefore = -1; *outToggled = 0; *outOnlyOne = 0;
    ...
    // Reshape into a flat terrain grid: ctrl[col][row] = (col*64, row*64, 0), edges un-turned.
    // Patch_TurnEdge gates on type == PATCH_TERRAIN (exact, not a bit test), so SET it.
    p->type = PATCH_TERRAIN;
    for ( int col = 0; col < p->width; ++col )
        for ( int row = 0; row < p->height; ++row )
        {
            p->ctrl[col][row].xyz[0] = (float)( col * 64 );
            p->ctrl[col][row].xyz[1] = (float)( row * 64 );
            p->ctrl[col][row].xyz[2] = 0.0f;
            p->ctrl[col][row].turned_edge = 0;
        }
    // Rebuild the curveDef as the terrain branch (verts mirror the control grid 1:1).
    if ( p->curveDef )
        free( p->curveDef );
    ...
    // Straight-down ray through cell (0,0) (x∈[0,64], y∈[0,64]); (16,16) lands clearly
    // inside the lower triangle (x+y < 64), away from the shared diagonal edge.
    float org[3] = { 16.0f, 16.0f, 100.0f };
    float dir[3] = {  0.0f,  0.0f,  -1.0f };

    *outBefore = p->ctrl[0][0].turned_edge & 1;          // expect 0

    Patch_TurnEdge( (int)(intptr_t)org, (int)(intptr_t)dir );

    *outToggled = ( ( p->ctrl[0][0].turned_edge & 1 ) != (unsigned)*outBefore ) ? 1 : 0;

    // Exactly ONE cell's diagonal should have changed.
    int changed = 0;
    for ( int col = 0; col < p->width; ++col )
        for ( int row = 0; row < p->height; ++row )
            if ( p->ctrl[col][row].turned_edge & 1 )
                ++changed;
    *outOnlyOne = ( changed == 1 ) ? 1 : 0;
    return 1;
}

// paintapply gate: the terrain-paint APPLY chain - sub_43E4B0 (0x43e4b0 brushlist loop) ->
// PMESH_16 (0x43ded0 per-control-point apply) -> sub_43DB60 (falloff) + sub_43E550 (raise
// callback) + the CurvEditDlg param getters.  The dialog is parked, so the control table
// is seeded directly (CurveEdit_SetCtrl).  With inner=0/outer=300/strength=1 a +64 stroke
// centred on ctrl[0][0] must raise the centre to ~64 and leave a far corner untouched.
typedef float (*PaintCb)( float *, int, float, float, float );
extern float sub_43E550( float *, int, float, float, float );                       // pmesh.cpp raise callback
extern void  sub_43E4B0( PaintCb cb, selbrush_t *list, char mask, float *center, float cellInfo ); // pmesh.cpp
extern void  CurveEdit_SetCtrl( int slot, int id, float value );                    // pmesh.cpp test setter

extern "C" int Radiant_TestPaintApply( int *outOk, float *outNearH, float *outFarH )
{
    *outOk = 0; *outNearH = 0; *outFarH = 0;

    int w0 = 0, h0 = 0, w1 = 0;
    if ( !Radiant_TestMakePatch( 0, &w0, &h0, &w1 ) )    // 9×3 cylinder (left selected)
        return 0;
    selbrush_t *sel = selected_brushes.next;
    if ( sel == &selected_brushes || !sel->def || !sel->def->patch )
        return 0;
    ...
    // Flat terrain grid: ctrl[col][row] = (col*64, row*64, 0).
    p->type = PATCH_TERRAIN;
    for ( int col = 0; col < p->width; ++col )
        for ( int row = 0; row < p->height; ++row )
        {
    ...
    // The symbiont brush bbox still reflects the original cylinder's CREATE position (which
    // varies per map); PMESH_16's paint-reach early-out (sub_4A56B0) tests it against the
    // stroke centre.  Set it to match the reshaped grid (corner at origin) so the stroke
    // reaches it deterministically on every map.
    sel->def->mins[0] = 0.0f; sel->def->mins[1] = 0.0f; sel->def->mins[2] = 0.0f;
    sel->def->maxs[0] = (float)( ( p->width - 1 ) * 64 );
    sel->def->maxs[1] = (float)( ( p->height - 1 ) * 64 );
    sel->def->maxs[2] = 0.0f;

    // Seed the CurvEditDlg params: inner radius 0 (1424), outer 300 (1425), strength 2^(8-8)=1 (1426).
    CurveEdit_SetCtrl( 0, 1424, 0.0f );
    CurveEdit_SetCtrl( 1, 1425, 300.0f );
    CurveEdit_SetCtrl( 2, 1426, 8.0f );

    float center[3] = { 0.0f, 0.0f, 0.0f };   // over ctrl[0][0]

    Undo_ClearRedo();
    Undo_GeneralStart( "paint terrain" );
    Undo_AddBrushList( &selected_brushes );
    sub_43E4B0( sub_43E550, &selected_brushes, 1 /*height*/, center, 64.0f /*+raise*/ );
    Undo_End();

    // Re-fetch the patch (Patch_Rebuild can swap pSymbiot, not the patchMesh_t).
    sel = selected_brushes.next;
    if ( sel == &selected_brushes || !sel->def || !sel->def->patch )
        return 0;
    p = sel->def->patch;

    float nearH = p->ctrl[0][0].xyz[2];                              // dist 0  -> full strength ~64
    float farH  = p->ctrl[p->width - 1][p->height - 1].xyz[2];       // dist ~528 > outer 300 -> ~0
    *outNearH = nearH;
    *outFarH  = farH;
    *outOk    = 1;
    return 1;
...
// softseldrag gate: the SOFT-SELECTION HEIGHT DRAG chain - sub_43DD00 (0x43dd00) ->
// PMESH_15 (0x43dca0 brushlist loop) -> sub_43DC10 (0x43dc10 per-control-point Z push) ->
// sub_43DA20 (0x43da20 falloff weight from d_move_points).  Same flat-terrain setup as
// paintapply, but ctrl[0][0] is registered as the single drag move-point and the drag is
// pushed +64 in Z; the move-point rises ~64 while a corner past the outer radius
extern void sub_43DD00( float amount );                              // pmesh.cpp soft-sel dispatch

extern "C" int Radiant_TestSoftSelDrag( int *outOk, float *outNearH, float *outFarH )
{
    *outOk = 0; *outNearH = 0; *outFarH = 0;

    int w0 = 0, h0 = 0, w1 = 0;
    if ( !Radiant_TestMakePatch( 0, &w0, &h0, &w1 ) )
        return 0;
    selbrush_t *sel = selected_brushes.next;
    if ( sel == &selected_brushes || !sel->def || !sel->def->patch )
        return 0;
    ...
    // Register ctrl[0][0] as the single drag move-point (the soft-sel falloff origin).
    g_qeglobals.d_move_points[0] = &p->ctrl[0][0];
    g_qeglobals.d_num_move_points = 1;

    // Seed the CurvEditDlg params: inner 0 (1424), outer 300 (1425), amplitude 2^(8-8)=1 (1426).
    CurveEdit_SetCtrl( 0, 1424, 0.0f );
    CurveEdit_SetCtrl( 1, 1425, 300.0f );
    CurveEdit_SetCtrl( 2, 1426, 8.0f );

    Undo_ClearRedo();
    Undo_GeneralStart( "soft-sel drag" );
    Undo_AddBrushList( &selected_brushes );
    sub_43DD00( 64.0f );                       // push the soft-selection up by 64 in Z
    Undo_End();
    g_qeglobals.d_num_move_points = 0;         // clear the gate's move-point
    ...
    *outNearH = p->ctrl[0][0].xyz[2];                              // at the move-point -> full weight ~64
    *outFarH  = p->ctrl[p->width - 1][p->height - 1].xyz[2];       // dist ~528 > outer 300 -> ~0
    *outOk    = 1;
    return 1;
}

// settexturing gate harness: Patch_SetTexturing (0x446b60) GRID mode — lay parametric texcoords
// across a flat WxH patch with sx=sy=4, mode=2, and assert the corners: ctrl[0][0]=(0,0),
// ctrl[w-1][0].s=4 (full sx across width), ctrl[0][h-1].t=-4 (full -sy down height).  Verifies
// the stride translation (ctrl[col][row].texCoord.st[2*layer]) of the texture-layout port.
extern void Patch_SetTexturing( float sx, float sy, int mode );                    // pmesh.cpp 0x446b60
extern "C" int Radiant_TestSetTexturing( int *outOk, float *outFarS, float *outFarT )
{
    *outOk = 0; *outFarS = 0; *outFarT = 0;
    ...
    Patch_SetTexturing( 4.0f, 4.0f, 2 );                          // GRID layout
    ...
    float nearS = p->ctrl[0][0].texCoord.st[2 * layer];                       // expect 0
    float nearT = p->ctrl[0][0].texCoord.st[2 * layer + 1];                   // expect 0
    *outFarS = p->ctrl[p->width - 1][0].texCoord.st[2 * layer];              // expect 4
    *outFarT = p->ctrl[0][p->height - 1].texCoord.st[2 * layer + 1];         // expect -4
    if ( nearS != 0.0f || nearT != 0.0f )
        return 0;
    *outOk = 1;
    return 1;
}

// patchcaptex gate: the cap/natural texturing chain - Patch_CapTexture (0x439b10) ->
// Patch_MeshNormals (0x437c80) + the PMESH_03 / Patch_GetAxisFace cap-face pick ->
// Patch_ST (0x4390d0) -> curveDef rebuild.  Drives BOTH cap paths (cap=0 PMESH_03,
// cap=1 Patch_GetAxisFace); asserts finite S/T, unit-length control normals, and a
// rebuilt curveDef with the version bumped.
extern curvePatchDef_t *Patch_CapTexture( patchMesh_t *p, char cap, float x, float y );  // pmesh.cpp 0x439b10
extern "C" int Radiant_TestPatchCapTex( int *outW, int *outH, int *outStFinite,
                                        int *outNormalsOk, int *outRegen )
{
    *outW = *outH = *outStFinite = *outNormalsOk = *outRegen = 0;
    ...
    *outW = p->width; *outH = p->height;
    ...
    // Make sure the symbiont bbox brush has windings (PMESH_03/Patch_GetAxisFace read
    // faces[].w); Patch_BrushToMesh built them on link, this is belt-and-braces.
    if ( p->pSymbiot )
        Brush_BuildWindings( (brush_t *)p->pSymbiot, 1 );
    ...
    // cap=0 → PMESH_03 reference-face path.
    Patch_CapTexture( p, 0, 0.5f, 0.5f );

    // S/T must be finite across the whole grid.
    int finite = 1;
    for ( int c = 0; c < p->width && finite; ++c )
        for ( int r = 0; r < p->height; ++r )
        {
            float s = p->ctrl[c][r].texCoord.st[2 * layer];
            float t = p->ctrl[c][r].texCoord.st[2 * layer + 1];
            if ( ( *(unsigned int *)&s & 0x7F800000 ) == 0x7F800000 ||
                 ( *(unsigned int *)&t & 0x7F800000 ) == 0x7F800000 ) { finite = 0; break; }
        }
    *outStFinite = finite;

    // Patch_MeshNormals must have left every control normal unit-length.
    int normalsOk = 1;
    for ( int c = 0; c < p->width && normalsOk; ++c )
        for ( int r = 0; r < p->height; ++r )
        {
            const float *n = p->ctrl[c][r].normal;
            float len = (float)sqrt( n[0]*n[0] + n[1]*n[1] + n[2]*n[2] );
            if ( len < 0.99f || len > 1.01f ) { normalsOk = 0; break; }
        }
    *outNormalsOk = normalsOk;

    *outRegen = ( p->curveDef != nullptr && p->version != verBefore ) ? 1 : 0;

    // cap=1 → Patch_GetAxisFace round-robin path: must not crash.
    Patch_CapTexture( p, 1, 0.25f, 0.25f );
    return 1;
}

// patchsamplesize gate: sub_442B00 (0x442b00), the patch sample-size setter reached by
// Brush_SetSampleSize/sub_477080 when a selected brush owns a patch.  Runs on layer 1 so
// the curveDef rebuild path executes; asserts the size stored, bDirty set, curveDef
// rebuilt and version bumped, then re-runs on the TERRAIN branch.
extern void sub_442B00( patchMesh_t *p, int sampleSize );   // pmesh.cpp 0x442b00
extern "C" int Radiant_TestPatchSampleSize( int *outW, int *outH, int *outSizeOk,
                                            int *outDirty, int *outRegen )
{
    *outW = *outH = *outSizeOk = *outDirty = *outRegen = 0;
    ...
    *outW = p->width; *outH = p->height;
    ...
    int savedLayer = g_qeglobals.current_edit_layer;
    g_qeglobals.current_edit_layer = 1;       // exercise the layer-1 curveDef rebuild path

    short verBefore = p->version;
    p->bDirty = 0;

    // snapshot the control-point S/T so we can confirm the PMESH_02 resample ran.
    float s0 = p->ctrl[2][1].texCoord.st[2 * 1];
    float t0 = p->ctrl[2][1].texCoord.st[2 * 1 + 1];

    sub_442B00( p, 64 );                        // set sample size 64 (runs the full PMESH_02 resample)

    float sz = *(float *)&p->size_of_struct_0x504C;
    *outSizeOk = ( sz > 63.9f && sz < 64.1f ) ? 1 : 0;
    *outDirty  = p->bDirty ? 1 : 0;
    *outRegen  = ( p->curveDef != nullptr && p->version != verBefore ) ? 1 : 0;

    // the resampled S/T must be FINITE across the whole grid, and the probe point's
    // coords must have actually changed (the cylinder is non-planar → the index-map spread).
    int finite = 1, changed = 0;
    for ( int c = 0; c < p->width && finite; ++c )
        for ( int r = 0; r < p->height; ++r )
        {
            float s = p->ctrl[c][r].texCoord.st[2 * 1];
            float t = p->ctrl[c][r].texCoord.st[2 * 1 + 1];
            if ( ( *(unsigned int *)&s & 0x7F800000 ) == 0x7F800000 ||
                 ( *(unsigned int *)&t & 0x7F800000 ) == 0x7F800000 ) { finite = 0; break; }
        }
    float s1 = p->ctrl[2][1].texCoord.st[2 * 1];
    float t1 = p->ctrl[2][1].texCoord.st[2 * 1 + 1];
    if ( s1 != s0 || t1 != t0 ) changed = 1;

    // Also exercise the TERRAIN branch (sub_442B00 → Patch_TerrainTexProject / sub_439350,
    // the dominant-axis projection): force PATCH_TERRAIN, re-tessellate, re-run, expect
    // finite S/T and no crash.
    p->type = PATCH_TERRAIN;
    if ( p->curveDef ) { free( p->curveDef ); p->curveDef = nullptr; }
    sub_442B00( p, 32 );
    int terrFinite = 1;
    for ( int c = 0; c < p->width && terrFinite; ++c )
        for ( int r = 0; r < p->height; ++r )
        {
            float s = p->ctrl[c][r].texCoord.st[2 * 1];
            float t = p->ctrl[c][r].texCoord.st[2 * 1 + 1];
            if ( ( *(unsigned int *)&s & 0x7F800000 ) == 0x7F800000 ||
                 ( *(unsigned int *)&t & 0x7F800000 ) == 0x7F800000 ) { terrFinite = 0; break; }
        }

    // fold all the resample checks into outRegen so the runner asserts them too.
    *outRegen = ( *outRegen && finite && changed && terrFinite ) ? 1 : 0;

    g_qeglobals.current_edit_layer = savedLayer;
    return 1;
}

// patchterrain gate: the TERRAIN branch of Patch_InsertRow (Patch_TerrainInsertRow
// 0x4436A0), never reached by the cylinder gates.  Verifies height 3->4, that the new
// row's smoothing is the midpoint of its two neighbours (the field the port used to
// drop), and that the new control points were appended to g_qeglobals.d_points.
extern void Patch_InsertRow( patchMesh_t *p, char flag );   // pmesh.cpp (0x443830)
extern "C" int Radiant_TestTerrainInsert( int *outH0, int *outH1, int *outSmoothOk, int *outPtsAdded )
{
    *outH0 = *outH1 = *outSmoothOk = *outPtsAdded = 0;
    ...
    if ( sel == &selected_brushes || !sel->def || !sel->def->patch )
        return 0;
    patchMesh_t *p = sel->def->patch;
    if ( p->width < 1 || p->height < 3 )
        return 0;

    p->type = (PATCH_TYPES)( (int)p->type | PATCH_TERRAIN );  // force the terrain branch
    for ( int c = 0; c < p->width; ++c )                      // distinctive smoothing on rows 0,1
    {
        p->ctrl[c][0].texCoord.smoothing[0] = 10.0f; p->ctrl[c][0].texCoord.smoothing[1] = 20.0f;
        p->ctrl[c][1].texCoord.smoothing[0] = 30.0f; p->ctrl[c][1].texCoord.smoothing[1] = 40.0f;
    }
    *outH0 = p->height;                                  // 3
    const int npBefore = g_qeglobals.d_numpoints;

    Patch_InsertRow( p, 0 );                             // terrain → Patch_TerrainInsertRow(at=1)
    *outH1 = p->height;                                  // expect 4

    int smoothOk = ( p->height == h0 + 1 );
    for ( int c = 0; c < p->width && smoothOk; ++c )     // new row 1 = midpoint(old row0, old row1)
    {
        float s0 = p->ctrl[c][1].texCoord.smoothing[0];  // expect (10+30)/2 = 20
        float s1 = p->ctrl[c][1].texCoord.smoothing[1];  // expect (20+40)/2 = 30
        float d0 = s0 - 20.0f; if ( d0 < 0 ) d0 = -d0;
        float d1 = s1 - 30.0f; if ( d1 < 0 ) d1 = -d1;
        if ( d0 > 0.01f || d1 > 0.01f ) smoothOk = 0;
    }
    *outSmoothOk = smoothOk;
    *outPtsAdded = g_qeglobals.d_numpoints - npBefore;   // expect width (9)
    return 1;
}

// patchprim gate: the NON-cylinder Patch_BrushToMesh primitive branches, which the `patch`
// gate never reaches.  Reports the control grid + patch type so the gate can assert the
// per-primitive geometry (bevel=3x3 PATCH_BEVEL, endcap=5x3 PATCH_ENDCAP, cone=9x3
// PATCH_CONE).
extern "C" int Radiant_TestMakePatchPrim( int cone, int bevel, int endcap, int square,
                                          int *outW, int *outH, int *outType )
{
    *outW = *outH = *outType = 0;
    ...
    Patch_BrushToMesh( (char)cone, (unsigned char)bevel, (unsigned char)endcap, (char)square );
...
// ─── curvedrag gate harness ──────────────────────────────────────────────────
// Exercises the patch control-point INTERACTIVE EDIT end-to-end headless:
//   Patch_EditPatch (enter sel_curvepoint + fill d_points[]) -> SelectCurvePointByRay
//   (SHIFT-ray through a control point -> PMESH_11 queues it) -> MoveSelection a known
//   world delta (-> Patch_UpdateSelected(_0) + Patch_Rebuild).
// Reports control-point markers, move-points picked, whether the first moved by the delta,
// and the tessellated vert count before/after.  0 = no editable world brush, -1 = failure.
extern void Patch_EditPatch();                                    // pmesh.cpp 0x441fd0
extern char g_bXYViewIsLastPatchClick;                            // pmesh.cpp byte_25D5A6A
extern void MoveSelection( float *origin, float *dir, float *move );      // drag.cpp 0x47f0c0
void SelectCurvePointByRay( int origin, int dir, unsigned int buttons );  // fwd (defined below)

extern "C" int Radiant_TestCurveDrag( int *outMarkers, int *outPicked, int *outMoved,
                                      int *outTessBefore, int *outTessAfter )
{
    *outMarkers = *outPicked = *outMoved = *outTessBefore = *outTessAfter = 0;
    ...
    if ( sel == &selected_brushes || !sel->def || !sel->def->patch )
        return -1;
    patchMesh_t *p = sel->def->patch;
    *outMarkers = p->width * p->height;
    *outTessBefore = p->curveDef ? ( p->curveDef->width * p->curveDef->height ) : 0;

    // (1) enter curve-point edit mode (fills g_qeglobals.d_points[] from the grid).
    Patch_EditPatch();
    if ( g_qeglobals.d_select_mode != sel_curvepoint || g_qeglobals.d_numpoints <= 0 )
        return -1;

    // (2) pick a control point by ray.  Aim a top-down (-Z) ray straight through a
    //     mid-grid control point so SelectCurvePointByRay's perpendicular-distance pick
    //     resolves it.  buttons = MK_LBUTTON|MK_SHIFT (1|4): SHIFT arms the ray-pick.
    //     (XY_MouseDown would normally set g_bXYViewIsLastPatchClick; force it 0 so the
    //     first plain bit is irrelevant — SHIFT is what matters here.)
    g_bXYViewIsLastPatchClick = 0;
    const drawVert_t *target = &p->ctrl[p->width / 2][p->height / 2];
    float start[3] = { target->xyz[0], target->xyz[1], target->xyz[2] + 131072.0f };
    float dir[3]   = { 0.0f, 0.0f, -1.0f };
    SelectCurvePointByRay( (int)(intptr_t)start, (int)(intptr_t)dir, 1 | 4 );
    *outPicked = g_qeglobals.d_num_move_points;
    if ( g_qeglobals.d_num_move_points <= 0 )
        return -1;                      // ray-pick selected nothing → wiring broken

    // Record the FIRST selected control point's pre-drag position.
    drawVert_t *mp0 = g_qeglobals.d_move_points[0];
    float pre[3] = { mp0->xyz[0], mp0->xyz[1], mp0->xyz[2] };

    // (3) drag it by a known world delta through the real MoveSelection path.
    float origin[3] = { start[0], start[1], start[2] };
    float move[3]   = { 64.0f, 0.0f, 0.0f };        // a clear outward X nudge
    MoveSelection( origin, dir, move );

    // moved exactly by the delta?  (Patch_UpdateSelected_0 adds `move` to each move-point.)
    float dx = mp0->xyz[0] - ( pre[0] + move[0] );
    float dy = mp0->xyz[1] - ( pre[1] + move[1] );
    float dz = mp0->xyz[2] - ( pre[2] + move[2] );
    *outMoved = ( dx * dx + dy * dy + dz * dz < 0.01f ) ? 1 : 0;

    // re-tessellated?  Patch_Rebuild rebuilt curveDef (a fresh allocation with the same
    // grid dims for a small in-plane nudge — the proof the tessellator re-ran).
    *outTessAfter = p->curveDef ? ( p->curveDef->width * p->curveDef->height ) : 0;

    if ( !*outMoved || *outTessAfter <= 0 )
        return -1;
    return 1;
}
...
// floordrop gate: the two NEW pieces of OnDropSelected - sub_47CFD0 (bbox pivot) and
// sub_47CBA0 (rotate-about-pivot).  Checks both pivot branches against the formula and
// that sub_47CBA0 is a no-op at 0 deg but bumps def->version at 90 deg.
extern "C" int Radiant_TestFloorDrop( int *outSnapOk, int *outRawOk, int *outRotOk )
{
    *outSnapOk = *outRawOk = *outRotOk = 0;
    selbrush_t *sel = selected_brushes.next;
    if ( sel == &selected_brushes || !sel->def || sel->def->faceCount < 1 )
        return 0;
    brush_t *def = sel->def;

    int   savedNoClamp = g_PrefsDlg->m_bNoClamp;
    float gs           = grid_sizes[g_qeglobals.d_gridsize];
    float mid[3];

    // clamp branch — midpoint snapped DOWN to the grid
    g_PrefsDlg->m_bNoClamp = 0;
    sub_47CFD0( mid, sel );
    int snapOk = 1;
    for ( int k = 0; k < 3; ++k )
    {
        float e = (float)floor( 0.5 * ( def->maxs[k] + def->mins[k] ) / gs ) * gs;
        if ( fabs( mid[k] - e ) > 0.01f ) snapOk = 0;
    }
    *outSnapOk = snapOk;

    // NoClamp branch — the raw midpoint
    g_PrefsDlg->m_bNoClamp = 1;
    sub_47CFD0( mid, sel );
    int rawOk = 1;
    for ( int k = 0; k < 3; ++k )
    {
        float e = ( def->maxs[k] - def->mins[k] ) * 0.5f + def->mins[k];
        if ( fabs( mid[k] - e ) > 0.01f ) rawOk = 0;
    }
    *outRawOk = rawOk;

    // sub_47CBA0 — 0 deg is a no-op, 90 deg about Z transforms (def->version bump)
    g_PrefsDlg->m_bNoClamp = savedNoClamp;
    int v0 = def->version;
    sub_47CBA0( sel, 2, 0.0f );
    int v1 = def->version;
    sub_47CBA0( sel, 2, 90.0f );
    *outRotOk = ( v1 == v0 && def->version > v1 ) ? 1 : 0;
...
#ifdef RADIANT_SELFTEST
// ═════════════════════════════════════════════════════════════════════════════
//  Radiant_TestMoveVertex - the VERTEX-EDIT gate.  Drives the whole ported chain:
//  select an editable world brush -> SetupVertexSelection builds the handle list (the real
//  MakeFace/Brush_MakeFaceWinding/FindPoint path) -> sel_vertex -> pick handle[0] as the
//  drag point -> MoveSelection applies a known OUTWARD diagonal delta -> Brush_MoveVertex
//  (0x471C30) re-triangulates the brush.  The delta is outward on each axis (one grid unit)
//  so an axis-aligned box stays convex and Brush_MoveVertex accepts the move on any map.
//  outHandles = -1 is the N/A sentinel (no editable inline world brush).
// ═════════════════════════════════════════════════════════════════════════════
extern void  MoveSelection( float *origin, float *dir, float *move );  // drag.cpp 0x47f0c0
extern "C" void *Radiant_TestMoveVertex( int *outHandles, int *outVertMoved,
                                         int *outFaceMoved, float *outDelta )
{
    *outHandles = *outVertMoved = *outFaceMoved = 0;
    outDelta[0] = outDelta[1] = outDelta[2] = 0.0f;

    // 1) pick an editable, non-patch world brush (same filter as the clip gate).
    selbrush_t *pick = nullptr;
    for ( selbrush_t *b = active_brushes.next; b && b != &active_brushes; b = b->next )
    {
        if ( b->patch || !b->def || b->def->faceCount <= 0 )
    ...
    if ( !pick )
    {
        *outHandles = -1;        // N/A: no editable inline world brush
        return nullptr;
    }
    ...
    // 2) enter vertex mode + build the handle list (the real subsystem under test).
    SetupVertexSelection();
    g_qeglobals.d_select_mode = sel_vertex;
    *outHandles = g_qeglobals.d_numpoints;
    if ( g_qeglobals.d_numpoints <= 0 )
    {
        g_qeglobals.d_select_mode = sel_brush;
        return nullptr;          // pick had no handles → real FAIL (handles=0)
    }

    // 3) pick handle[0] as the single drag move-point (what SelectVertexByRay does
    //    on a hit; chosen directly here for determinism).
    g_qeglobals.d_num_move_points = 0;
    g_qeglobals.d_move_points[g_qeglobals.d_num_move_points++] =
        (drawVert_t *)&g_qeglobals.d_points[0];

    float startV[3] = { g_qeglobals.d_points[0][0],
                        g_qeglobals.d_points[0][1],
                        g_qeglobals.d_points[0][2] };

    // 4) OUTWARD diagonal delta of one grid unit (keeps an axis-aligned box convex).
    float center[3] = {
        0.5f * ( pick->def->mins[0] + pick->def->maxs[0] ),
        0.5f * ( pick->def->mins[1] + pick->def->maxs[1] ),
        0.5f * ( pick->def->mins[2] + pick->def->maxs[2] ) };
    float gs = grid_sizes[g_qeglobals.d_gridsize];
    float delta[3];
    for ( int a = 0; a < 3; ++a )
        delta[a] = ( startV[a] >= center[a] ? gs : -gs );
    outDelta[0] = delta[0]; outDelta[1] = delta[1]; outDelta[2] = delta[2];

    // expected snapped end position (Brush_MoveVertex grid-snaps end = start + delta).
    float expEnd[3];
    for ( int a = 0; a < 3; ++a )
        expEnd[a] = (float)( floor( ( startV[a] + delta[a] ) / gs + 0.5 ) * gs );

    // 5) DRAG: the real MoveSelection sel_vertex dispatch → Brush_MoveVertex.
    float origin[3] = { 0, 0, 0 }, dir[3] = { 0, 0, 1 };
    MoveSelection( origin, dir, delta );

    // 6a) did the picked vertex end up at the snapped target?
    const float *endV = g_qeglobals.d_move_points[0]->xyz;
    bool vmoved = ( fabs( endV[0] - expEnd[0] ) < 0.1
                 && fabs( endV[1] - expEnd[1] ) < 0.1
                 && fabs( endV[2] - expEnd[2] ) < 0.1
                 && ( fabs( endV[0] - startV[0] ) > 0.01
                   || fabs( endV[1] - startV[1] ) > 0.01
                   || fabs( endV[2] - startV[2] ) > 0.01 ) );
    *outVertMoved = vmoved ? 1 : 0;

    // 6b) did at least one face's planepts carry the moved point (geometry updated)?
    int faceMoved = 0;
    for ( int fi = 0; fi < pick->def->faceCount && !faceMoved; ++fi )
    {
        face_t *f = &pick->def->faces[fi];
        for ( int pj = 0; pj < 3; ++pj )
            if ( fabs( f->planepts[pj][0] - expEnd[0] ) < 0.1
              && fabs( f->planepts[pj][1] - expEnd[1] ) < 0.1
              && fabs( f->planepts[pj][2] - expEnd[2] ) < 0.1 )
            { faceMoved = 1; break; }
    }
    *outFaceMoved = faceMoved;

    return pick->def;
}

// ═════════════════════════════════════════════════════════════════════════════
//  Radiant_TestMoveEdge - the EDGE-EDIT gate.  SetupVertexSelection builds d_points +
//  d_edges -> sel_edge -> Select_Edge (0x494dc0) with a ray at edge[0]'s midpoint ->
//  SelectFaceEdge (0x494c30) rewrites both shared faces' planepts and queues the two
//  endpoints as move-points -> MoveSelection through the generic (!= sel_vertex) branch.
//  The delta is outward on the two axes PERPENDICULAR to the edge and zero on the edge
//  axis, so the box stays convex and the move is accepted on any map.
extern "C" void *Radiant_TestMoveEdge( int *outEdges, int *outMovePts,
                                       int *outBothMoved, int *outFaceMoved,
                                       float *outDelta )
{
    *outEdges = *outMovePts = *outBothMoved = *outFaceMoved = 0;
    outDelta[0] = outDelta[1] = outDelta[2] = 0.0f;

    // 1) pick an editable, non-patch world brush (same filter as the vertex gate).
    selbrush_t *pick = nullptr;
    for ( selbrush_t *b = active_brushes.next; b && b != &active_brushes; b = b->next )
    {
        if ( b->patch || !b->def || b->def->faceCount <= 0 )
    ...
    if ( !pick )
    {
        *outEdges = -1;          // N/A: no editable inline world brush
        return nullptr;
    }
    ...
    // 2) enter edge mode + build the handle lists (the real subsystem under test).
    SetupVertexSelection();
    g_qeglobals.d_select_mode = sel_edge;
    *outEdges = g_qeglobals.d_numedges;
    if ( g_qeglobals.d_numedges <= 0 )
    {
        g_qeglobals.d_select_mode = sel_brush;
        return nullptr;          // pick had no edges → real FAIL (edges=0)
    }

    // 2b) GATE-ONLY: drop any NON-manifold edge (f1 or f2 unset) from d_edges before the
    //     pick, mirroring the real "only fully-shared edges are pickable" set so the ray
    //     pick is deterministic and every survivor is safe to drag.  For a clean box this
    //     is a no-op, and it touches only the gate's view of d_edges.
    {
        int w = 0;
        for ( int r = 0; r < g_qeglobals.d_numedges; ++r )
            if ( g_qeglobals.d_edges[r].f1 && g_qeglobals.d_edges[r].f2 )
                g_qeglobals.d_edges[w++] = g_qeglobals.d_edges[r];
        g_qeglobals.d_numedges = w;
    }
    if ( g_qeglobals.d_numedges <= 0 )
    {
        g_qeglobals.d_select_mode = sel_brush;
        return nullptr;          // no manifold edge on this brush (degenerate geometry)
    }

    // 3) edge[0] (the first manifold edge) midpoint + endpoints (the d_points it spans).
    const pedge_t *e0 = &g_qeglobals.d_edges[0];
    float A[3] = { g_qeglobals.d_points[e0->p1][0], g_qeglobals.d_points[e0->p1][1],
                   g_qeglobals.d_points[e0->p1][2] };
    float B[3] = { g_qeglobals.d_points[e0->p2][0], g_qeglobals.d_points[e0->p2][1],
                   g_qeglobals.d_points[e0->p2][2] };
    float mid[3] = { 0.5f * ( A[0] + B[0] ), 0.5f * ( A[1] + B[1] ), 0.5f * ( A[2] + B[2] ) };

    // 4) PERPENDICULAR-OUTWARD delta of one grid unit (zero on the edge axis).
    float center[3] = {
        0.5f * ( pick->def->mins[0] + pick->def->maxs[0] ),
        0.5f * ( pick->def->mins[1] + pick->def->maxs[1] ),
        0.5f * ( pick->def->mins[2] + pick->def->maxs[2] ) };
    ...
    for ( int a = 0; a < 3; ++a )
    {
        if ( (float)fabs( A[a] - B[a] ) > 0.1f )
            delta[a] = 0.0f;                                   // edge axis → don't slide along it
        else
            delta[a] = ( mid[a] >= center[a] ? gs : -gs );     // push outward perpendicular
    }
    outDelta[0] = delta[0]; outDelta[1] = delta[1]; outDelta[2] = delta[2];

    // 5) PICK the edge with a real ray through its midpoint (origin = mid, dir = +Z):
    //    perpendicular distance to edge[0] is 0, and Select_Edge's strict `best>dist`
    //    keeps the lowest-index edge on a tie, so edge[0] always wins → its two faces'
    //    edges resolve via SelectFaceEdge and the two endpoints get queued.
    float origin[3] = { mid[0], mid[1], mid[2] };
    float dir[3]    = { 0.0f, 0.0f, 1.0f };
    Select_Edge( (int)(intptr_t)dir, (int)(intptr_t)origin );
    int nmp = g_qeglobals.d_num_move_points;
    *outMovePts = nmp;
    // Each shared face contributes the edge's TWO endpoints (SelectFaceEdge writes them
    // into THAT face's planepts and AddPlanept queues the per-face planept POINTER, with no
    // cross-face dedup), so a manifold edge shared by 2 faces yields 4 move-points.
    if ( nmp < 2 || nmp > 16 )
    {
        g_qeglobals.d_select_mode = sel_brush;
        return nullptr;          // Select_Edge/SelectFaceEdge did not queue the endpoints
    }

    // snapshot every queued move-point's start world position (the snapped planepts).
    float startP[16][3];
    for ( int i = 0; i < nmp; ++i )
    {
        startP[i][0] = g_qeglobals.d_move_points[i]->xyz[0];
        startP[i][1] = g_qeglobals.d_move_points[i]->xyz[1];
        startP[i][2] = g_qeglobals.d_move_points[i]->xyz[2];
    }

    // the move-points must span at least TWO distinct world positions (a real edge, not a
    // single repeated vertex) — find the two snapped endpoint worlds (endptA / endptB).
    float endptA[3] = { startP[0][0], startP[0][1], startP[0][2] };
    bool haveB = false;
    float endptB[3] = { 0, 0, 0 };
    for ( int i = 1; i < nmp; ++i )
        if ( (float)fabs( startP[i][0] - endptA[0] ) > 0.1f
          || (float)fabs( startP[i][1] - endptA[1] ) > 0.1f
          || (float)fabs( startP[i][2] - endptA[2] ) > 0.1f )
        { endptB[0] = startP[i][0]; endptB[1] = startP[i][1]; endptB[2] = startP[i][2]; haveB = true; break; }
    if ( !haveB )
    {
        g_qeglobals.d_select_mode = sel_brush;
        return nullptr;          // degenerate — the queued points are a single vertex
    }

    // 6) DRAG: the real MoveSelection sel_edge dispatch (generic move-points branch).
    MoveSelection( origin, dir, delta );

    // 6a) did EVERY queued move-point move by exactly the delta? (the whole edge as a unit)
    bool nonZero = ( fabs( delta[0] ) > 0.01 || fabs( delta[1] ) > 0.01 || fabs( delta[2] ) > 0.01 );
    bool allMoved = nonZero;
    for ( int i = 0; i < nmp; ++i )
    {
        const float *e = g_qeglobals.d_move_points[i]->xyz;
        if ( fabs( e[0] - ( startP[i][0] + delta[0] ) ) >= 0.1
          || fabs( e[1] - ( startP[i][1] + delta[1] ) ) >= 0.1
          || fabs( e[2] - ( startP[i][2] + delta[2] ) ) >= 0.1 )
        { allMoved = false; break; }
    }
    *outBothMoved = allMoved ? 1 : 0;

    // 6b) does a face's planepts carry each moved endpoint world (geometry updated)?
    float expA[3] = { endptA[0] + delta[0], endptA[1] + delta[1], endptA[2] + delta[2] };
    float expB[3] = { endptB[0] + delta[0], endptB[1] + delta[1], endptB[2] + delta[2] };
    int gotA = 0, gotB = 0;
    for ( int fi = 0; fi < pick->def->faceCount; ++fi )
    {
        face_t *f = &pick->def->faces[fi];
        for ( int pj = 0; pj < 3; ++pj )
        {
            if ( fabs( f->planepts[pj][0] - expA[0] ) < 0.1
              && fabs( f->planepts[pj][1] - expA[1] ) < 0.1
              && fabs( f->planepts[pj][2] - expA[2] ) < 0.1 ) gotA = 1;
            if ( fabs( f->planepts[pj][0] - expB[0] ) < 0.1
              && fabs( f->planepts[pj][1] - expB[1] ) < 0.1
              && fabs( f->planepts[pj][2] - expB[2] ) < 0.1 ) gotB = 1;
        }
    }
    *outFaceMoved = ( gotA && gotB ) ? 1 : 0;

    return pick->def;
}

// ═════════════════════════════════════════════════════════════════════════════
//  Radiant_TestDragClose - the DRAG-CLOSE gate.  Protects drag.cpp's Drag_MouseUp
//  vertex/edge close path, which IDA 0x48064d gates on `mode==sel_edge||sel_vertex` to call
//  sub_4718D0 (Select_RemoveEmptyFaces): drop null-winding faces, then bump def->version
//  (>=4 faces survive) or free the brush (<4).  No other gate drives Drag_MouseUp.
//  On a clean (convex, >=4-face) brush the removal is a no-op, so the ++version branch runs
//  exactly once: version +1, faceCount unchanged, mode still sel_edge.
extern void        Undo_Start( const char *op );        // undo.cpp
...
extern "C" void *Radiant_TestDragClose( int *outVerDelta, int *outFaceDelta,
                                        int *outModeReset )
{
    *outVerDelta = -999; *outFaceDelta = -999; *outModeReset = 0;

    // pick a CLEAN editable world brush: >=4 faces, every face has a winding (so the
    // close-path's empty-face removal is a guaranteed no-op and the result is determinate
    // — exactly the ++version branch, faceCount unchanged).
    selbrush_t *pick = nullptr;
    for ( selbrush_t *b = active_brushes.next; b && b != &active_brushes; b = b->next )
    {
        if ( b->patch || !b->def || b->def->faceCount < 4 )
            continue;
        if ( *(int *)&( (entity_s_def *)b->owner->def )->eclass->fixedsize )
            continue;
        bool allWound = true;
        for ( int fi = 0; fi < b->def->faceCount; ++fi )
            if ( !b->def->faces[fi].w ) { allWound = false; break; }
        if ( !allWound )
            continue;
        pick = b;
        break;
    ...
    if ( !pick )
        return nullptr;          // N/A: pure-prefab / no editable inline world brush

    Select_Deselect( 1 );
    Select_Brush( pick, 1, 0, 0 );

    // enter edge mode + open the undo record exactly as Drag_Setup's sel_edge branch.
    SetupVertexSelection();
    g_qeglobals.d_select_mode = sel_edge;
    Undo_Start( "drag edge" );
    Undo_AddBrushList( &selected_brushes );

    short verBefore  = pick->def->version;
    int   faceBefore = pick->def->faceCount;

    // the unit under test: Drag_MouseUp's vertex/edge undo-close → Select_RemoveEmptyFaces.
    Drag_MouseUp( 1 /*MK_LBUTTON*/ );

    *outVerDelta   = (int)( (short)pick->def->version - verBefore );
    *outFaceDelta  = pick->def->faceCount - faceBefore;
    // Drag_MouseUp does NOT reset edge/vertex mode (faithful: the user stays in the mode
    // for the next drag) — it only resets the marquee/area modes. Report whether the edge
    // mode persisted (the expected, faithful outcome).
    *outModeReset  = ( g_qeglobals.d_select_mode == sel_edge ) ? 1 : 0;
    g_qeglobals.d_select_mode = sel_brush;   // restore for any later harness in the run
    return pick->def;
}

// ═════════════════════════════════════════════════════════════════════════════
//  Radiant_TestCloneRotate - the SELECTION-TRANSFORMS gate.  Clone_Selection must add
//  exactly ONE brush with def refCount == 2 (def-list + display-instance) and leave the
//  clone selected; rotating it 90 degrees about Z (Select_GetMid -> Select_RotateAxis(2,90)
//  -> Select_ApplyMatrix_SelectedBrushes) must change its planepts and keep it a valid
//  convex brush (faceCount >= 4, every face wound by Brush_BuildWindings).
//  Returns the clone DEF; outAdded = -1 means N/A (no editable inline world brush).
extern "C" void *Radiant_TestCloneRotate( int *outAdded, int *outAddedRefcount,
                                          int *outRotated, int *outExactRot )
{
    *outAdded = 0; *outAddedRefcount = 0; *outRotated = 0;
    if ( outExactRot ) *outExactRot = 0;

    // count ALL brushes (active + selected) before the clone — Clone_Selection moves
    // the clone into selected_brushes, so an active-only count would read unchanged.
    int beforeN = 0;
    for ( selbrush_t *b = active_brushes.next;   b && b != &active_brushes;   b = b->next ) ++beforeN;
    for ( selbrush_t *b = selected_brushes.next; b && b != &selected_brushes; b = b->next ) ++beforeN;

    // 1) pick an editable, non-patch world brush (same filter as the clip/vertex gates).
    selbrush_t *pick = nullptr;
    for ( selbrush_t *b = active_brushes.next; b && b != &active_brushes; b = b->next )
    {
        if ( b->patch || !b->def || b->def->faceCount <= 0 )
    ...
    if ( !pick )
    {
        *outAdded = -1;          // N/A: no editable inline world brush
        return nullptr;
    }
    ...
    // 2) CLONE the selection. The clone replaces the selection (active+selected total grows).
    Clone_Selection( grid_sizes[g_qeglobals.d_gridsize] );

    int afterN = 0;
    for ( selbrush_t *b = active_brushes.next;   b && b != &active_brushes;   b = b->next ) ++afterN;
    for ( selbrush_t *b = selected_brushes.next; b && b != &selected_brushes; b = b->next ) ++afterN;
    *outAdded = afterN - beforeN;     // expect +1 (one brush cloned)

    // The clone is now the (single) selected brush.
    selbrush_t *clone = selected_brushes.next;
    if ( clone == &selected_brushes || !clone->def )
        return nullptr;              // clone path failed to (re)select
    *outAddedRefcount = clone->def->refCount;   // expect 2

    // snapshot the clone's first-face planepts before rotation
    float pre[3][3];
    for ( int i = 0; i < 3; ++i )
        for ( int j = 0; j < 3; ++j )
            pre[i][j] = clone->def->faces[0].planepts[i][j];

    // 3) ROTATE the clone 90° about Z (the exact Brush→Rotate→Z menu path).
    float rot_around[4][3];
    Select_GetMid( rot_around[0] );
    Select_RotateAxis( 2, 90.0f, (float (*)[4][3])rot_around );
    // capture the rotation pivot (rot_around[0] = Select_GetMid, grid-snapped) for the
    // EXACT-coordinate check below.  Select_RotateAxis only fills rows 1..3, leaving the
    // pivot in row 0 untouched, so this is the mid the apply will rotate about.
    const float mid[3] = { rot_around[0][0], rot_around[0][1], rot_around[0][2] };
    Select_ApplyMatrix_SelectedBrushes( 0, rot_around[0], 90.0f, 0 );

    // did the planepts change AND is the brush still valid?
    bool changed = false;
    for ( int i = 0; i < 3 && !changed; ++i )
        for ( int j = 0; j < 3; ++j )
            if ( fabs( clone->def->faces[0].planepts[i][j] - pre[i][j] ) > 0.01 )
            { changed = true; break; }

    bool valid = ( clone->def->faceCount >= 4 );
    for ( int fi = 0; fi < clone->def->faceCount && valid; ++fi )
        if ( !clone->def->faces[fi].w )
            valid = false;

    *outRotated = ( changed && valid ) ? 1 : 0;

    // EXACT-COORDINATE check, derived from Select_RotateAxis (0x48FF40, axis=Z) +
    // Select_ApplyMatrix (0x47CDE0) -> OrientationPosToWorldPos (0x4BA430): SinCosDeg(-90)
    // (0x4AABC0) gives sin=-1, cos=0 exactly (the 270 special case), so the rotation rows
    // are (0,-1,0)/(1,0,0)/(0,0,1) and world[j] = mid[j] + sum_i axis[i][j]*(p[i]-mid[i])
    // collapses to x' = mid.x + (p.y-mid.y), y' = mid.y - (p.x-mid.x), z' = p.z.  Bit-exact
    // for integer planepts; the epsilon absorbs ULP wobble on far-from-origin brushes.
    bool exact = true;
    for ( int i = 0; i < 3 && exact; ++i )
    {
        const float ex = mid[0] + ( pre[i][1] - mid[1] );
        const float ey = mid[1] - ( pre[i][0] - mid[0] );
        const float ez = pre[i][2];
        const float gx = clone->def->faces[0].planepts[i][0];
        const float gy = clone->def->faces[0].planepts[i][1];
        const float gz = clone->def->faces[0].planepts[i][2];
        const float tolx = (float)( fabs( ex ) * 1e-5 ) + 0.01f;
        const float toly = (float)( fabs( ey ) * 1e-5 ) + 0.01f;
        const float tolz = (float)( fabs( ez ) * 1e-5 ) + 0.01f;
        if ( fabs( gx - ex ) > tolx || fabs( gy - ey ) > toly || fabs( gz - ez ) > tolz )
            exact = false;
    }
    if ( outExactRot ) *outExactRot = exact ? 1 : 0;
    return clone->def;
}

// ═════════════════════════════════════════════════════════════════════════════
//  Radiant_TestCopyPaste - the CLIPBOARD COPY/PASTE round-trip gate.  Drives the in-app
//  clipboard (RadiantClipboard_Copy -> Map_ImportBuffer via RadiantClipboard_Paste, the
//  engine behind CXYWnd::Copy/Paste).  Paste deselects, places a DUPLICATE worldspawn brush
//  on world_entity and selects it: exactly ONE new brush, pasted def refCount == 2, and the
//  geometry must match the copied brush.
// ═════════════════════════════════════════════════════════════════════════════
// C++ linkage (entity.cpp) — must be declared OUTSIDE the extern "C" harness fn below,
// or they would inherit C linkage and fail to resolve the C++ definitions.
void RadiantClipboard_Copy();    // entity.cpp
void RadiantClipboard_Paste();   // entity.cpp

extern "C" void *Radiant_TestCopyPaste( int *outAdded, int *outAddedRefcount,
                                        int *outMatched )
{
    *outAdded = 0; *outAddedRefcount = 0; *outMatched = 0;

    int beforeN = 0;
    for ( selbrush_t *b = active_brushes.next;   b && b != &active_brushes;   b = b->next ) ++beforeN;
    for ( selbrush_t *b = selected_brushes.next; b && b != &selected_brushes; b = b->next ) ++beforeN;

    // 1) pick an editable, non-patch brush whose planepts are GRID-ALIGNED (integer).
    //    Paste re-runs Brush_BuildWindings(.,bFull=1), which Brush_SnapPlanepts-snaps the
    //    pasted brush to the grid (faithful to Map_ImportBuffer), so only an already-
    //    on-grid brush round-trips bit-exact.
    selbrush_t *pick = nullptr;
    for ( selbrush_t *b = active_brushes.next; b && b != &active_brushes; b = b->next )
    {
        if ( b->patch || !b->def || b->def->faceCount <= 0 )
        ...
        for ( int f = 0; f < b->def->faceCount && gridAligned; ++f )
            for ( int i = 0; i < 3 && gridAligned; ++i )
                for ( int j = 0; j < 3; ++j )
                {
                    float v = b->def->faces[f].planepts[i][j];
                    if ( fabs( v - floor( v + 0.5 ) ) > 0.01 ) { gridAligned = false; break; }
                }
        if ( !gridAligned )
            continue;
        pick = b;
    ...
    if ( !pick )
    {
        *outAdded = -1;          // N/A: no editable grid-aligned inline brush
        return nullptr;
    }
    ...
    // 2) COPY the selection to the in-app clipboard.
    RadiantClipboard_Copy();

    // 3) PASTE (Map_ImportBuffer): deselects, places + selects a duplicate worldspawn brush.
    RadiantClipboard_Paste();

    int afterN = 0;
    for ( selbrush_t *b = active_brushes.next;   b && b != &active_brushes;   b = b->next ) ++afterN;
    for ( selbrush_t *b = selected_brushes.next; b && b != &selected_brushes; b = b->next ) ++afterN;
    *outAdded = afterN - beforeN;     // expect +1 (one brush pasted)

    // The pasted brush is now the (single) selected brush.
    selbrush_t *pasted = selected_brushes.next;
    if ( pasted == &selected_brushes || !pasted->def )
        return nullptr;              // paste path failed to (re)select
    *outAddedRefcount = pasted->def->refCount;   // expect 2

    // GEOMETRY MATCH.  A brush's identity is its plane SET + bounds, independent of face
    // storage order, so the order-independent invariant is: (a) same faceCount, (b)
    // identical AABB (recomputed by Brush_BuildWindings from the real geometry), and (c)
    // every original face-plane present on SOME pasted face.  pick->def is still live in
    // active_brushes (paste only deselected it).
    brush_t *od = pick->def, *nd = pasted->def;
    bool matched = ( nd->faceCount == od->faceCount );
    for ( int k = 0; k < 3 && matched; ++k )
    {
        if ( fabs( nd->mins[k] - od->mins[k] ) > 0.01 ) matched = false;
        if ( fabs( nd->maxs[k] - od->maxs[k] ) > 0.01 ) matched = false;
    }
    // (c) each original face's planepts exist on some pasted face (any order).
    for ( int of = 0; of < od->faceCount && matched; ++of )
    {
        bool found = false;
        for ( int nf = 0; nf < nd->faceCount && !found; ++nf )
        {
            bool same = true;
            for ( int i = 0; i < 3 && same; ++i )
                for ( int j = 0; j < 3; ++j )
                    if ( fabs( nd->faces[nf].planepts[i][j] - od->faces[of].planepts[i][j] ) > 0.01 )
                    { same = false; break; }
            if ( same ) found = true;
        }
...
// ═════════════════════════════════════════════════════════════════════════════
//  Radiant_TestMarquee - the MARQUEE / REGION-SELECT gate.  Drives
//  Select_FlipFilteredBrushes (sub_4902C0) with a KNOWN world box (the reference brush's
//  X/Y bounds inflated by +1, full Z) and checks that EXACTLY the brushes the box covers
//  are selected - against an independently computed oracle - then that Select_Delete
//  removes only those.  Returns 0 = N/A (no editable world brush).
extern "C" int Radiant_TestMarquee( int *outExpected, int *outSelected,
                                    int *outRefSelected, int *outDeleted )
{
    *outExpected = 0; *outSelected = 0; *outRefSelected = 0; *outDeleted = 0;

    // headless: no active XY window → Region_ViewAxis* default to XY (axes X=0, Y=1).
    // Compute the two tested axes the SAME way the core does, so the oracle matches.
    const int axA = Region_ViewAxisA();
    const int axB = Region_ViewAxisB();

    // 1) pick the first editable, non-patch, non-fixed-size world brush as the reference.
    selbrush_t *ref = nullptr;
    for ( selbrush_t *b = active_brushes.next; b && b != &active_brushes; b = b->next )
    {
        if ( b->patch || !b->def || b->def->faceCount <= 0 )
            continue;
    ...
    if ( !ref )
        return 0;   // N/A: pure-prefab / no editable inline world brush

    // box = reference X/Y bounds inflated by +1 unit on the two screen axes, full depth.
    float boxMins[3] = { -131072.0f, -131072.0f, -131072.0f };
    float boxMaxs[3] = {  131072.0f,  131072.0f,  131072.0f };
    boxMins[axA] = ref->def->mins[axA] - 1.0f;  boxMaxs[axA] = ref->def->maxs[axA] + 1.0f;
    boxMins[axB] = ref->def->mins[axB] - 1.0f;  boxMaxs[axB] = ref->def->maxs[axB] + 1.0f;

    // 2) ORACLE: count active brushes the box CONTAINS on the two screen axes, applying
    //    the SAME eligibility filter the core uses (FilterBrush + brushFlags 2/0x20).
    int expected = 0;
    for ( selbrush_t *b = active_brushes.next; b && b != &active_brushes; b = b->next )
    {
        brush_t *def = b->def;
        if ( !def ) continue;
        if ( boxMaxs[axA] >= def->maxs[axA] && boxMins[axA] <= def->mins[axA]
          && boxMaxs[axB] >= def->maxs[axB] && boxMins[axB] <= def->mins[axB]
          && !FilterBrush( b, 0 ) )
        {
            int bf = b->brushFlags;
            if ( (bf & 2) == 0 && (bf & 0x20) == 0 )
    ...
    *outExpected = expected;
    ...
    // 3) DRIVE the marquee core (mode 12 = box ADD: active → selected).
    Select_Deselect( 1 );

    // total world brushes (active + selected) BEFORE the marquee — the delete must remove
    // exactly the selected subset from this total (affects-only-those proof).
    int totalBefore = 0;
    for ( selbrush_t *b = active_brushes.next;   b && b != &active_brushes;   b = b->next ) ++totalBefore;
    for ( selbrush_t *b = selected_brushes.next; b && b != &selected_brushes; b = b->next ) ++totalBefore;

    Select_FlipFilteredBrushes( boxMins, boxMaxs, 1 );
    ...
    {
        ++selN;
        if ( b == ref ) refSel = true;
    }
    *outSelected    = selN;
    *outRefSelected = refSel ? 1 : 0;

    // 4) DELETE the selection → frees exactly the selected subset; the surviving world
    //    (active + selected) total must be totalBefore - selN.
    Select_Delete();
    int totalAfter = 0;
    for ( selbrush_t *b = active_brushes.next;   b && b != &active_brushes;   b = b->next ) ++totalAfter;
    for ( selbrush_t *b = selected_brushes.next; b && b != &selected_brushes; b = b->next ) ++totalAfter;
    *outDeleted = totalBefore - totalAfter;     // brushes the delete removed (expect selN)
