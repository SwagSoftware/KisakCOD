...
char __cdecl G_MoverPush(gentity_s *pusher, float *move, float *amove, gentity_s **obstacle)
{
    ...
    *obstacle = 0;
    ...
    for (j = 0; j < v9; ++j)
    {
        ...
        if ((ent->s.eType == 4 || ent->s.eType == ET_ITEM || ent->s.eType == ET_PLAYER || ent->physicsObject)
    ...
    for (j = 0; j < v10; ++j)
    {
        ...
        if (G_TryPushingEntity(ent, pusher, move, amove) || ent->s.eType == ET_ITEM || ent->s.eType == 4)
        {
            SV_LinkEntity(ent);
        }
        else
