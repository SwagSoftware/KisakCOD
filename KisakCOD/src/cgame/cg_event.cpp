...
void __cdecl CG_EntityEvent(int localClientNum, centity_s *cent, int event)
{
    ...
    if (event)
    {
        ...
        if (event <= 0)
            MyAssertHandler(".\\cgame\\cg_event.cpp", 517, 0, "%s\n\t(event) = %i", "(event > 0)", event);
        if (event >= 135)
            MyAssertHandler(".\\cgame\\cg_event.cpp", 518, 0, "%s\n\t(event) = %i", "(event < EV_MAX_EVENTS)", event);
        if (cg_debugEvents->current.enabled)
        ...
        if ((cgameGlob->bgs.clientinfo[clientNum].perks & 0x100) != 0)
            ...
            offset = 0;
        if (event < 77 || event > 105)
        {
            if (event < 106 || event > 134)
            {
                switch (event)
                {
                case 1:
