    ...
    GfxCmdBufContext context)
{
    ...
    __int64 worldMata; // [esp+5Ch] [ebp-B8h]
    float worldMat_4; // [esp+60h] [ebp-B4h]
    float worldMat_8; // [esp+64h] [ebp-B0h]
    __int64 worldMat_8a; // [esp+64h] [ebp-B0h]
    float worldMat_16; // [esp+6Ch] [ebp-A8h]
    __int64 worldMat_16a; // [esp+6Ch] [ebp-A8h]
    float worldMat_20; // [esp+70h] [ebp-A4h]
    float worldMat_24; // [esp+74h] [ebp-A0h]
    __int64 worldMat_24a; // [esp+74h] [ebp-A0h]
    float worldMat_32; // [esp+7Ch] [ebp-98h]
    __int64 worldMat_32a; // [esp+7Ch] [ebp-98h]
    float worldMat_36; // [esp+80h] [ebp-94h]
    float worldMat_40; // [esp+84h] [ebp-90h]
    __int64 worldMat_40a; // [esp+84h] [ebp-90h]
    __int64 worldMat_48; // [esp+8Ch] [ebp-88h]
    __int64 worldMat_56; // [esp+94h] [ebp-80h]
    unsigned int baseGfxEntIndex; // [esp+A0h] [ebp-74h]
    unsigned __int64 drawSurf; // [esp+A4h] [ebp-70h]
    ...
    drawSurfMask.packed = -16777216;
    drawSurfKey = drawSurf & 0xFFFFFFFFFF000000uLL;
    drawSurfIndex = 0;
    ...
    while (1)
    {
        ...
        scale.v[2] = scale.v[0];
        scale.v[3] = scale.v[0];
        *&worldMata = worldMat * scale.v[0];
        *(&worldMata + 1) = worldMat_4 * scale.v[0];
        *&worldMat_8a = worldMat_8 * scale.v[0];
        *(&worldMat_8a + 1) = 0.0 * scale.v[0];
        *&worldMat_16a = worldMat_16 * scale.v[0];
        *(&worldMat_16a + 1) = worldMat_20 * scale.v[0];
        *&worldMat_24a = worldMat_24 * scale.v[0];
        *(&worldMat_24a + 1) = *(&worldMat_8a + 1);
        *&worldMat_32a = worldMat_32 * scale.v[0];
        *(&worldMat_32a + 1) = worldMat_36 * scale.v[0];
        *&worldMat_40a = worldMat_40 * scale.v[0];
        *(&worldMat_40a + 1) = *(&worldMat_8a + 1);
        *&worldMat_48 = modelSurf->placement.base.origin[0] - eyeOffset.v[0];
        *(&worldMat_48 + 1) = modelSurf->placement.base.origin[1] - eyeOffset.v[1];
        *&worldMat_56 = modelSurf->placement.base.origin[2] - eyeOffset.v[2];
        *(&worldMat_56 + 1) = 0.0 - eyeOffset.v[3];
        matrix = R_GetActiveWorldMatrix(context.source);
        *&matrix->matrices.matrix[0].m[0][0] = worldMata;
        *&matrix->matrices.matrix[0].m[0][2] = worldMat_8a;
        *&matrix->matrices.matrix[0].m[1][0] = worldMat_16a;
        *&matrix->matrices.matrix[0].m[1][2] = worldMat_24a;
        *&matrix->matrices.matrix[0].m[2][0] = worldMat_32a;
        *&matrix->matrices.matrix[0].m[2][2] = worldMat_40a;
        *&matrix->matrices.matrix[0].m[3][0] = worldMat_48;
        *&matrix->matrices.matrix[0].m[3][2] = worldMat_56;
        R_SetModelLightingCoordsForSource(modelSurf->surf.info.lightingHandle, context.source);
        R_SetReflectionProbe(context, BYTE2(drawSurf));
    ...
    while (1)
    {
        ...
        *&matrix->matrices.matrix[0].m[3][2] = worldMat_56;
        ...
        if (++drawSurfIndex == drawSurfCount || (drawSurfMask.packed & drawSurfList[drawSurfIndex].packed) != drawSurfKey)
            break;
        modelSurf = (const GfxModelRigidSurface*)(data + 4 * *&drawSurfList[drawSurfIndex].packed);
        gfxEntIndex = modelSurf->surf.info.gfxEntIndex;
        if (gfxEntIndex != baseGfxEntIndex)
        {
            if (gfxEntIndex)
    ...
    GfxCmdBufContext context)
{
    ...
    float worldMat; // [esp+5Ch] [ebp-A0h]
    __int64 worldMata; // [esp+5Ch] [ebp-A0h]
    float worldMat_4; // [esp+60h] [ebp-9Ch]
    float worldMat_8; // [esp+64h] [ebp-98h]
    __int64 worldMat_8a; // [esp+64h] [ebp-98h]
    float worldMat_16; // [esp+6Ch] [ebp-90h]
    __int64 worldMat_16a; // [esp+6Ch] [ebp-90h]
    float worldMat_20; // [esp+70h] [ebp-8Ch]
    float worldMat_24; // [esp+74h] [ebp-88h]
    __int64 worldMat_24a; // [esp+74h] [ebp-88h]
    float worldMat_32; // [esp+7Ch] [ebp-80h]
    __int64 worldMat_32a; // [esp+7Ch] [ebp-80h]
    float worldMat_36; // [esp+80h] [ebp-7Ch]
    float worldMat_40; // [esp+84h] [ebp-78h]
    __int64 worldMat_40a; // [esp+84h] [ebp-78h]
    __int64 worldMat_48; // [esp+8Ch] [ebp-70h]
    __int64 worldMat_56; // [esp+94h] [ebp-68h]
